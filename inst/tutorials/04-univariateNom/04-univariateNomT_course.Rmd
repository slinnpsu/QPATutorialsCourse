...
Before computing proportions, check whether the grouping variable has any missing values. If it does, they form a group of their own and land in the denominator, making the substantive proportions sum to less than 1. **TrumpMajority** has a recorded value for every county, so there is nothing to set aside here. You will meet a variable that does need attention later in this tutorial.
...
# "Black", "Hispanic", "Other/No", "White" in that order.
```
...
grade_this({
  ...
  expected_labels <- c("Black", "Hispanic", "Other/No", "White")

  if (!identical(x_scale$labels, expected_labels)) {
    fail("Use the shortened labels \"Black\", \"Hispanic\", \"Other/No\", and \"White\" in that order.")
  }
...
The `ggplot()` call itself is unchanged: **df** is named directly in `data =`, as **counties** was earlier. When a variable does need filtering first, the data are piped in instead and `data =` disappears, because the pipe supplies it --- you will see that pattern in the regime type example later in this tutorial.
...
grade_this({
  ...
  expected_labels <- c("Dictatorship", "Parliamentary democracy", "Presidential democracy")
  if (!identical(x_scale$labels, expected_labels)) {
    fail("Use the category labels \"Dictatorship\", \"Parliamentary democracy\", and \"Presidential democracy\" in that order.")
  }
...
ggplot(data = counties,
  ...
  labs(title = "Number of US Counties by Racial Majority",
       ...
       y = "Frequency",
       caption = "Source: Linn, Nagler, Zilinsky") +
  scale_x_discrete(labels = c("Other/No", "Hispanic", "Black", "White")) +
  coord_flip()
```
...
The table below collects the tools for describing a nominal variable, and everything that follows explains why those are the ones available and how to produce them in R. Tutorials 5 and 6 will add columns of their own as ordinal and interval variables make more measures available.
...
In R, you implemented the analysis using a [dplyr]{.package-name} pipeline. Load each package once at the top of your script or Quarto document with `library(dplyr)` --- you do not need to repeat the call each time you use a [dplyr]{.package-name} function. Both `summarise()` and `summarize()` are valid spellings in [dplyr]{.package-name} and produce identical results.

You used the `group_by()` function to separate observations into categories, the `summarise()` function with `freq = n()` to count cases in each group, producing a [**frequency table**]{.important-text}, and the `mutate()` function with `prop = freq / sum(freq)` to convert counts to proportions.

You visualized distributions using [ggplot2]{.package-name}, loaded once with `library(ggplot2)`. Bar charts are built layer by layer: the `ggplot()` function sets up the data and maps the variable to the x-axis, the `geom_bar()` function adds a [**geom**]{.important-text} layer that counts cases and draws bars, the `labs()` function adds titles and axis labels, and the `scale_x_discrete()` function replaces stored values with meaningful category labels and suppresses missing values with `na.translate = FALSE` when needed. Use the `factor()` function when a variable is stored as numeric codes but should be treated as categorical; character variables and variables already stored as factors do not need it.
