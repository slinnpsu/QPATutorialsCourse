...
question("Based on the frequency table, why is **TrumpMajority** = 1 the modal category?",
  ...
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "The mode is determined by one column of the table. Look at the counts, not the codes."
)
```
...
question("Based on the frequency table, which statement best describes the distribution of counties?",
  ...
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Ask which statement a reader who does not have the codebook could actually use."
)
```
...
**Run and observe 5: Calculate proportions using `mutate()`**

```{r analysis-calculate-proportions, exercise=TRUE, warning=FALSE, exercise.cap="Run and observe 5"}
counties %>%
...
**Practice 6: Interpret the proportion table**
...
**Practice 7: Create a proportion table**
...
```{r analysis-practice-proportions, exercise=TRUE, exercise.cap="Practice 7"}
counties %>%
...
**Practice 8: Identify the mode of racial_majority**
...
**Practice 9: Summarize the data by region**
...
```{r analysis-practice-freq-by-region, exercise=TRUE, exercise.lines=6, exercise.cap="Practice 9"}
...
# Hint 2 of 2
# Use df and region. Remove missing values first.
# Then group by region, count the states in each group
# in a column named freq, and create prop by dividing
# each frequency by the sum of all frequencies.
```
...
**Practice 10: Describe the distribution of region**
...
**Practice 11: Characterize the dispersion**
...
**Practice 12: Classify a distribution shape**
...
**Run and observe 13: Set up the plot and map the variable**

```{r visual-initialize-plot, exercise=TRUE, exercise.cap="Run and observe 13"}
library(ggplot2)
...
**Run and observe 14: Add the bar layer**

```{r visual-add-geom-bar, exercise=TRUE, exercise.cap="Run and observe 14"}
ggplot(data = counties, mapping = aes(x = factor(TrumpMajority))) +
  geom_bar()
```
...
**Run and observe 15: Add labels**

```{r visual-add-labs, exercise=TRUE, exercise.cap="Run and observe 15"}
ggplot(data = counties, mapping = aes(x = factor(TrumpMajority))) +
  geom_bar() +
...
**Run and observe 16: Replace the axis tick labels**

```{r visual-customize-scale-x, exercise=TRUE, exercise.cap="Run and observe 16"}
ggplot(data = counties, mapping = aes(x = factor(TrumpMajority))) +
  geom_bar() +
  labs(title = "Number of US Counties that Cast a Majority of \nVotes for the Major Party Presidential Candidates",
       subtitle = "2016",
...
**Practice 17: What does a bar chart add?**
...
**Practice 18: Create a bar chart for racial majority**

Fill in the three blanks to complete the chart for **racial_majority**. The shortened category labels should drop the word "Majority" --- "Black", "Hispanic", "Other", "White" --- listed in alphabetical order, which is the order they are stored in the data. One hint is available. Click "Submit Answer" to check your work.

```{r visual-practice-bar-race, exercise=TRUE, exercise.cap="Practice 18"}
...
**Run and observe 19: Build a proportion bar chart for Census region**

```{r visual-bar-proportion-region, exercise=TRUE, exercise.cap="Run and observe 19"}
df %>%
  filter(!is.na(region)) %>%
...
**Practice 20: Interpret the proportion bar chart**
...
**Run and observe 21: Check the storage type and levels of dem_level4**

```{r visual-regime-class, exercise=TRUE, exercise.cap="Run and observe 21"}
...
**Practice 22: Create a proportion bar chart for regime type**
...
```{r visual-practice-bar-proportion-regime, exercise=TRUE, exercise.cap="Practice 22"}
...
**Practice 23: Describe the shape of the regime type distribution**
...
**Explore 24: Reorder categories and flip coordinates**

```{r visual-reorder-and-flip, exercise=TRUE, exercise.cap="Explore 24"}
...
**Apply 26: Choose the right tool**
...
**Apply 27: Identify the distribution shape**
