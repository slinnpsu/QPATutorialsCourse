...
question("Which of the following variables are nominal level measures? Select all that apply.",
  ...
  answer("The race of a respondent in a survey", correct = TRUE,
         message = "The race variable is nominal --- its values are unordered category labels. No arrangement of racial categories carries any inherent meaning. Nominal variables are described using frequency tables and bar charts --- the only tools that make sense when values are labels with no order. Means, medians, and other statistics that require ranking or spacing are not applicable."),
  answer("The status of the death penalty (permitted or not) in a state", correct = TRUE,
         message = "The death penalty status variable is nominal --- permitted and not permitted are unordered categories with no meaningful rank. Like all nominal variables, it is described using frequency tables and bar charts, not means or medians."),
...
To illustrate how to compute a frequency table and identify the mode, you will examine the variable **TrumpMajority** in the **counties** data frame. This variable assigns a value of 0 to counties where Clinton won a majority of the vote and 1 to counties where Trump won a majority.
...
question("Based on the proportion table you just created, which statement best describes the distribution of states across regions?",
  correct = "Right. No single region clearly dominates.",
  ...
  answer("No single region clearly dominates --- the states are spread fairly evenly across all four regions", correct = TRUE,
         message = "While the South has the most states, the other regions have 9 to 13. With counts ranging from 9 to 16 across four regions, the distribution is fairly even --- the mode is a less informative summary here than it was for TrumpMajority or racial_majority, and reporting the full table is the only way to give an accurate picture of the distribution."),
...
::: {.tip}
**How ggplot2 works:** Every ggplot is built by layering components on top of each other using `+`. Each layer adds something new to the plot. You will always start with the `ggplot()` function to set up the plot, then add layers to specify what to draw and how to label it.
:::
...
The first line loads the [ggplot2]{.package-name} package, which is not part of base R. Like [dplyr]{.package-name} earlier in this tutorial, it must be loaded with `library()` before its functions are available. Once loaded, it remains available for the rest of the R session, which is why subsequent exercises do not repeat the call.

The `ggplot()` function tells ggplot two things: which data to use and which variable to map to a visual feature. We use explicit argument names --- `data =` and `mapping =` --- because the `ggplot()` function takes several arguments and naming them makes clear which is which. This is good practice any time a function call is complex enough that the purpose of each argument might otherwise be unclear.

The `mapping` argument takes an `aes()` function call. The `aes()` function --- short for aesthetics --- is how we tell ggplot which variable maps to which visual feature of the plot. Here we set `x = factor(TrumpMajority)`, which places the categories of **TrumpMajority** along the x-axis. We specify x and not y because for a bar chart the categories go on the x-axis and ggplot computes the bar heights on the y-axis automatically --- we do not need to tell it what to count.

Because **TrumpMajority** is coded as 0 and 1, we wrap it in the `factor()` function to tell R it is categorical rather than numeric. Without this, ggplot might treat the values as a continuous scale rather than two distinct categories. The `factor()` function is only needed when a variable is stored as numbers but should be treated as categories. Character variables --- those stored as text strings like "Black Majority" or "South" --- are already treated as discrete categories by ggplot without any wrapping. Variables already stored as factors in R also do not need wrapping. In practice, checking how a variable is stored before writing plotting code will tell you whether `factor()` is needed: if it is numeric and categorical, wrap it; if it is character or already a factor, you can map it directly.
...
::: {.tip}
**Always type quotation marks yourself.** When you copy text from a tutorial page into R code, the quotation marks may look the same but are actually different characters --- browsers render "curly" quotes (" ") that R cannot read, instead of the straight quotes (" ") that R requires. If you copy a label or title string and get an unexpected syntax error, this is almost certainly the cause. Always type your own quotation marks when writing R code.
:::
...
- Caption: "Source: Linn, Nagler, Zilinsky"
- Shortened category labels dropping the word "Majority": "Black", "Hispanic", "Other", "White" (listed in the order they are stored in the data, which for this variable is alphabetical)
...
# Hint 3 of 3
# The trickiest part is scale_x_discrete().
# racial_majority is stored as a character variable,
# so ggplot already treats it as categorical --- no
# factor() needed. In scale_x_discrete(),
# na.translate = FALSE suppresses the missing-value
# bar, and labels = c(...) replaces the stored
# category names with the shortened versions.
# List them in the order they are stored in the data,
# which for this variable is alphabetical.
```
...
So far our bar charts have shown frequencies --- counts of how many counties fall in each category. Sometimes we want to show proportions instead. The reason comes down to what bar height actually represents in a frequency chart: it reflects both how common a category is *and* how large the dataset is. That means two distributions that look identical in shape can produce bars of very different heights just because one dataset has more cases. 

Imagine comparing the distribution of racial majority across US counties to the same distribution across a smaller set of 100 precincts. The county bars would tower over the precinct bars even if the two distributions were otherwise identical --- not because the pattern is different, but simply because there are more counties. Proportions remove that dependency by putting every distribution on a common 0-to-1 scale, so bar height reflects only each category's share of its own total. This makes proportion bar charts the right choice when the total number of cases matters less than each category's share of the whole, and essential when comparing distributions across datasets or groups of different sizes. 

To produce proportion bars, we add an aesthetic inside the `geom_bar()` function that transforms the y-axis: `aes(y = after_stat(prop), group = 1)`. We can also fill the bars with a color using the `fill` argument outside the `aes()` call.

The `ggplot()` function call, the `labs()` function, and the `scale_x_discrete()` function are unchanged from a frequency bar chart. The only difference is the `geom_bar()` layer. Three things inside that layer are new and worth understanding before you try it yourself.

`after_stat(prop)` tells ggplot to compute the proportion of cases in each category after the data have been tallied, rather than using a raw column from the data frame. It transforms the y-axis from a count scale to a proportion scale automatically. `group = 1` is needed because ggplot must know which observations to group together when computing the proportions. Without it, ggplot would compute each bar's proportion relative to itself --- every bar would show 1.0. Setting `group = 1` tells ggplot to treat all bars as part of the same group and compute each category's share of the whole. Finally, `fill = "blue"` sits outside the `aes()` call because it is a fixed setting applied equally to all bars, not a mapping from a variable in the data. When a visual property is constant rather than variable, it belongs outside `aes()`. Study the structure below, then try it yourself.
...
The pipeline and labels for a proportion chart of **region** are set up below. Add the correct `geom_bar()` layer to produce proportion bars filled in blue. This exercise focuses on the one part of the code that changes when moving from a frequency bar chart to a proportion bar chart: the bar height should represent each category's share of all cases, not the raw count. Click "Submit Answer" to check your work. One hint is available.
...
grade_this({
  ...
  pass("Exactly right! The key change from a frequency chart is the aes(y = after_stat(prop), group = 1) inside geom_bar(). The ggplot() function call, labs(), and any scale functions stay the same.")
})
```
...
question("A researcher codes political party affiliation as 1 = Democrat, 2 = Republican, 3 = Independent, 4 = Other, and reports that the mean party affiliation across survey respondents is 2.3. What is wrong with this?",
  ...
  answer("Nothing --- the mean is always an appropriate summary of central tendency.",
         message = "The mean requires a numeric scale with equal intervals and a meaningful zero point. Party affiliation has neither: the codes 1 through 4 are arbitrary labels, not quantities. A mean of 2.3 implies that the distance between Democrat and Republican is the same as between Republican and Independent, which is not a property of the categories --- it is an artifact of the coding scheme."),
...
- [**Dispersion**]{.important-text} describes how spread out the cases are across categories. For a nominal variable you assess dispersion by examining how dominant the modal category is: a mode that accounts for 92% of cases describes the distribution far more precisely than one that accounts for only slightly more than the others. Because the mode alone may not convey the full picture, the entire frequency or proportion distribution is often presented alongside it.
...
In R, you implemented the analysis using a [dplyr]{.package-name} pipeline. Load the package once per session with `library(dplyr)` --- you do not need to repeat the call each time you use a dplyr function. Both `summarise()` and `summarize()` are valid spellings in [dplyr]{.package-name} and produce identical results.
