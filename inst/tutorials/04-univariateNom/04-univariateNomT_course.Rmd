...
Univariate description means examining one variable at a time. You will use the same framework each time:

::: {.important-note}
**The three dimensions of univariate description**

Every variable you examine will be described along three dimensions:
...
Once the groups have been defined, the next step is to summarize each one. The `summarise()` function collapses each group into a single row of summary information. You tell it what to compute and what to call the result. To count the number of counties in each group you use `n()`, which simply counts rows, and store the result in a variable called **freq**. Run the code below and look at the output.
...
Knowing that 2,622 counties went for Trump is informative, but it is harder to interpret than knowing the *proportion* of counties that did so. Proportions tell you how informative the mode is on its own. If the modal category contains 51% of cases, it is technically the most common category, but nearly half the cases fall elsewhere. If it contains 84% of cases, the modal category accounts for most of the observations. Proportions become especially important later on when comparing variables across data sets of different sizes.
...
grade_this({
  ...
  pass("Correct! Your proportion table is complete --- examine the **prop** column carefully before answering the next question. Which category has the highest proportion, and how does it compare to the others?")
})
```
...
question("Based on the proportion table you just created, which statement best describes the central tendency of **racial_majority**?",
  ...
  answer("The modal category is White Majority", correct = TRUE,
         message = "White Majority is by far the most common category, making it the clear mode. With about 92% of counties in this category, the mode captures an unusually large share of the distribution --- a county drawn at random would be very likely to be White Majority. How much the mode tells you always depends on how dominant the modal category turns out to be."),
...
How informative the mode is on its own depends on how dominant the modal category is. When a large majority of cases falls in that category, the mode captures an important feature of the distribution. When the modal category contains only slightly more cases than the others, you need the full distribution to see what the mode leaves out.
...
How concentrated or spread out a distribution is determines how informative the mode is on its own. When one category dominates, the mode captures a central feature of the distribution. When cases are spread more evenly, the full distribution becomes essential context --- the mode identifies the most common category but leaves much of the picture untold.
...
```{r visual-bar-proportion-region, exercise=TRUE, exercise.cap="Run and observe 18", fig.alt="Proportion bar chart of US states by Census region, with four blue bars of broadly similar height. The y-axis runs from 0 to about 0.3 and is labeled Proportion, so no region accounts for much more than a third of states."}
df %>%
  filter(!is.na(region)) %>%
...
```{r visual-practice-bar-proportion-regime, exercise=TRUE, exercise.cap="Practice 21"}
