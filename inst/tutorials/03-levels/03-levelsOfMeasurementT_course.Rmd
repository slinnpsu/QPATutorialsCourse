...
The three levels form a hierarchy, and what accumulates is the set of summaries each one supports. Naming the most common category needs only labels, so it works for any variable. Finding the middle value needs the values to be ordered, so it works for ordinal and interval variables but not nominal ones. Averaging needs the distances between values to be equal, which only interval variables give you. So an interval variable can be summarized in any of these ways, an ordinal variable in the first two, and a nominal variable only in the first. Using fewer of a variable's properties than you have is always legitimate --- you are declining information, not misusing it.
...
question("Which of the following variables are interval (or ratio) level measures? Select all that apply.",
  ...
  answer("The level of foreign direct investment in a country measured in dollars", correct = TRUE,
         message = "Dollars are the plainest interval case: an increase of one million dollars is the same size whether a country starts near zero or in the billions."),
...
question("What level of measurement would a variable be if it was coded as the percentage of the vote cast for Donald Trump?",
  correct = "Right. Same concept as the last question, measured differently --- and a different level of measurement as a result.",
  answer("Nominal",
...
question("When an analyst treats an ordinal variable as if it were interval-level, what assumption is the analyst making?",
  correct = "Right. Common and often defensible --- but always an assumption, and one worth stating explicitly.",
...
The level of measurement of a variable determines how we describe its central features and how we analyze relationships between variables. Every method in this series begins with this step: identifying what kind of variable we are working with.

Three levels cover everything you will meet in these tutorials. A [**nominal**]{.important-text} variable takes unordered category labels --- rearranging them changes nothing. An [**ordinal**]{.important-text} variable takes values with a meaningful order, but with no guarantee that adjacent steps are the same size. An [**interval**]{.important-text} variable takes ordered values whose differences are equal throughout the scale, which is what makes subtraction meaningful.
...
<table class="qpa-table narrow-label">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>GDP and anything else measured in dollars; the unemployment rate; a count of protest events</td>
    </tr>
  </tbody>
</table>
:::
