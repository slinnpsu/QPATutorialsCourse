...
Over the course of this semester, we will be describing individual variables and analyzing (describing and testing hypotheses about) the relationship between variables. How we do so will depend on the level of measurement each variable takes. Every method we use and every hypothesis test we conduct will be determined by the level of measurement that characterizes the data. This is where levels of measurement come in. The [**level of measurement**]{.important-text} refers to how and how precisely a variable is measured. Generally, we distinguish between three levels of measurement: **nominal**, **ordinal**, and **interval**. (Interval variables that have a true zero point are sometimes called ratio variables, but in this course we treat ratio as a subtype of interval and use the term interval throughout.)
...
When you are unsure, slow down and ask what the values represent. Are they labels, ranks, or numbers with equal distances between them?
...
**Practice: Identify the nominal-level variables**
...
question("Which of the following variables are nominal level measures? Select all that apply.",
  ...
  answer("Whether an individual has not completed high school, has a high school degree, has attended some college, has a college degree, or has a post-secondary education or beyond",
...
**Practice: Identify the ordinal-level variables**
...
**Practice: Identify the interval-level variables**
...
**Practice: Identify the measurement level for a binary majority indicator**
...
question("What level of measurement would a variable be if it was coded as whether or not a majority of the county vote was cast for Donald Trump?",
  correct = "Right. A variable that takes only two unordered values --- majority for Trump or not --- is nominal. Because there is no sense in which one outcome is higher or lower than the other, only different, there is no rank order and therefore no basis for calling this ordinal or interval. Two-category nominal variables are also called binary or dichotomous.",
  answer("Ordinal",
...
**Practice: Identify the measurement level for a percentage**
...
question("What level of measurement would a variable be if it was coded as the percentage of the vote cast for Donald Trump?",
  correct = "Right. A percentage measured this way is interval-level because the values are ordered and the distance between any two points is equal throughout the scale. A county at 60% gave exactly 10 points more of the vote to Trump than a county at 50% --- that holds everywhere on the scale. Notice the contrast with the previous question: the same underlying concept measured as a binary indicator is nominal, but measured as a continuous percentage it is interval.",
  answer("Nominal",
...
**Practice: Classify the 1--7 ideology scale**
...
question("What level of measurement would a variable measuring ideology have if it was coded based on responses to the question: On a scale of 1 to 7, where 1 is very liberal and 7 is very conservative, where would you place yourself?",
  correct = "Right. The 1-to-7 ideology scale is technically ordinal because we cannot assume that the psychological distance between adjacent points is equal --- moving from 3 to 4 may not represent the same ideological shift as moving from 6 to 7. However, as the next section discusses, analysts frequently treat multi-point scales like this as interval-level, which requires acknowledging that equal-interval assumption explicitly.",
  answer("Nominal",
...
**Practice: Identify the measurement level for a voting-with-leadership percentage**
...
question("What level of measurement would a variable measuring ideology have if the variable was coded as the percent of times a member of Congress voted with the Republican leadership?",
  correct = "Right. This is another example of the same concept measured at a different level. A percentage is interval-level because the values are ordered and a one-point difference means the same thing throughout the scale --- a member voting with leadership 70% of the time is exactly 10 points above one voting with leadership 60% of the time, and that 10-point gap is the same size anywhere on the scale.",
  answer("Nominal",
         message = "The values this variable takes are ordered and the differences between them are meaningful, so it is not nominal. Nominal values are unordered category labels."),
  answer("Ordinal",
...
One final complication is worth naming directly: analysts sometimes treat an ordinal variable as if it were measured at the interval level. Consider the level of education coded as whether an individual has not completed high school, has a high school degree, has attended some college, has a college degree, or has a post-secondary education or beyond. Often analysts treat the differences between categories *as if* they are equal. This is particularly common when the number of ranked values a variable takes is large. In some cases such treatment may be justified, but it is important to recognize that in treating an ordinal variable as an interval-level variable, an analyst is assuming that the differences between values are equal.


## Check Your Understanding

The following questions ask you to apply the distinction across measurement levels. Think about what the variable's values mean before choosing an answer. If you get one wrong, use the feedback to identify which part of the distinction tripped you up.

**Apply: Distinguish ordinal and interval variables**
...
question("What is the key difference between an ordinal variable and an interval variable?",
  correct = "Right. Both ordinal and interval variables have ordered values --- that is what separates them from nominal variables. The critical distinction is equal distances. For interval variables, a one-unit difference means the same thing anywhere on the scale. For ordinal variables, the gaps between adjacent values are not guaranteed to be equal, so arithmetic operations on the values are not justified.",
...
**Apply: Compare measures of the same concept**
...
question("Suppose the concept is support for stricter gun laws. Which pair correctly classifies two different ways to measure that concept?",
  correct = "Right. The same concept can be operationalized at different levels of measurement. A binary support/oppose item produces a nominal variable because the two values are unordered categories. A percentage of survey respondents produces an interval variable because the values are ordered and equal distances are guaranteed. Neither is inherently better --- the choice depends on what you want to measure and what questions you want to answer.",
...
**Apply: Recognize an assumption about ordinal variables**
...
question("When an analyst treats an ordinal variable as if it were interval-level, what assumption is the analyst making?",
  correct = "Right. When an analyst treats an ordinal variable as interval-level, the assumption is that the distances between ordered categories are close enough to equal that treating them as such will not seriously distort the analysis. This is common and often defensible in political science --- particularly for scales with many ordered points, like a 7-point ideology scale --- but it is always an assumption that should be stated explicitly, not a fact that can be taken for granted.",
...
A reliable way to classify a variable is to ask the same questions each time. Are the values category labels with no natural order? If so, the variable is nominal. If the values can be ranked, ask whether the distances between values are meaningful and equal. If the answer is no, the variable is ordinal. If the answer is yes, the variable is interval-level.
...
<p class="tutorial-complete"
   role="status"
   aria-label="Nicely done! You have completed this tutorial and can now identify and distinguish levels of measurement, a foundation you will use throughout the course.">
