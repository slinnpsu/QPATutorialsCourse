...
What is a variable? A [**variable**]{.important-text} records a characteristic that can differ across cases. Analysts often use variables to measure broader concepts they care about. For example, the concept of political participation might be measured with variables such as whether someone voted, how often they attend meetings, or how much they donated to a campaign. Variables are meant to record characteristics that can differ across cases. If every case in a particular data set has the same value, the variable is constant in those data. The values are often numeric, even when those numbers stand in for labels. Variables may be classified by their level of measurement.


## Levels of Measurement

Across this series we will describe individual variables and analyze relationships between variables. The appropriate method depends on how each variable is measured. The [**level of measurement**]{.important-text} tells us what the values of a variable mean: whether they are unordered categories, ordered categories, or numbers with equal distances between them. In this series we distinguish among three levels of measurement: **nominal**, **ordinal**, and **interval**. (Interval variables that have a true zero point are sometimes called ratio variables, but in these tutorials we treat ratio as a subtype of interval and use the term interval throughout.)
