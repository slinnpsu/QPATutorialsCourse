...
Every analysis begins the same way. You open a file, find out what is in it, and get oriented before doing anything else. What follows is that sequence, walked through once and slowly: reading data in, learning what a data set contains, working with one variable at a time, handling the values that are missing, locating particular observations, and finally creating new variables and saving your work. The individual commands matter, but the routine matters more --- it is what you will repeat every time you open a data set you have not seen before.

In this tutorial you will learn:

* How to read R data files (.RData) into R using the `load()` function
* How to read comma-separated (.csv), Excel (.xlsx), Stata (.dta), and tab-delimited (.txt) files using the `import()` function from the [rio]{.package-name} package
...
The next four subsections show the four file types you will encounter most often. The syntax is nearly identical in each case --- only the filename changes. Work through them in order the first time, then treat this section as a reference you can return to.
...
question("Which of the following does summary() NOT report for a numeric variable?",
  ...
  answer("Standard deviation", correct = TRUE,
         message = "summary() does not report the standard deviation. It gives you the five-number summary (min, 1st quartile, median, 3rd quartile, max) plus the mean --- enough to see the shape and center of a distribution at a glance. But if you need a measure of spread, you will have to call sd() separately. This matters in practice: the mean and standard deviation together are the inputs to many of the statistical tests you will meet later."),
  answer("Mean",
...
Count the missing values in **likes_count** to confirm. Three hints are available. Click "Submit Answer" to check your work.
...
Since **shares_count** has no missing values, the result here is the same with or without `na.rm = TRUE`. When you call `mean()` on a variable that contains missing values, you will need this argument to compute a mean from the values that are present. What is worth pausing over is not whether to type it but what the resulting number describes: it is the mean among the cases that have values, not among all of them. That is why counting the missing values first is worth the few seconds it takes.
...
grade_this({
  ...
  pass(paste0("Good work! Posts received an average of ",
              ...
              " likes among the posts that have a recorded value. That is what na.rm = TRUE gives you --- the mean across the cases with values, not across all 1,643 posts."))
})
```
...
Read the result and notice what kind of content it is. Posts about major humanitarian events or global milestones tend to generate the most engagement --- a pattern you will see repeatedly when you analyze social media data. You will use this pattern frequently to identify which state, country, or case has the highest or lowest value of a given variable.
...
How many posts tied for the fewest likes (**likes_count**) of any post in the data set? Include `na.rm = TRUE`. Three hints are available. Click "Submit Answer" to check your work.
...
grade_this({
  ...
  pass(paste0("Exactly --- ", correct, " posts tied at ", fewest,
              " likes. A tie that large is worth pausing over. Every one of those posts is also missing its message text. That connection is worth noticing, though the data alone cannot tell you why those posts drew no likes. Noticing patterns like this in the data is part of the work."))
})
```
...
How many posts received more shares (**shares_count**) than likes (**likes_count**)? Combine `>` with `sum()` to count them, and include `na.rm = TRUE`. Four hints are available. Click "Submit Answer" to check your work.
...
question("A student runs `counties <- load(\"counties2016.RData\")` and then runs `nrow(counties)`, which returns `NULL` instead of a number. What went wrong?",
  ...
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Think about what `load()` returns, and about what the assignment operator stores."
)
```
