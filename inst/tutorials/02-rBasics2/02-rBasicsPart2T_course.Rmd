...
- Compare values using `==`, `>`, and `<`, and count the results with `sum()`
...
Stata data files have the extension .dta. The pattern is the same as before: load the package, import the file, assign the result, and print it.
...
**Practice 5: Import a Stata data file**

Write the code to import "oilwomen.dta," assign it to an object called **oil**, and print it. Three hints are available. Click "Submit Answer" to check your work.

```{r data3-import-csv, exercise=TRUE, exercise.setup="data3-import-csv-setup", exercise.cap="Practice 5"}
...
Tab-delimited .txt files are imported the same way.
...
**Practice 6: Import a tab-delimited file**

Import "turnout2.txt," assign it to an object called **TO**, and print it. You may assume the [rio]{.package-name} package is already loaded. Three hints are available. Click "Submit Answer" to check your work.

```{r data4-import-csv, exercise=TRUE, exercise.setup="data4-import-csv-setup", exercise.cap="Practice 6"}
...
**Practice 7: Identify the class of unFBposts**

Determine the class of **unFBposts**. Three hints are available. Click "Submit Answer" to check your work.

```{r data-explore-class, exercise=TRUE, exercise.cap="Practice 7"}
...
**Run and observe 8: Count the rows**

```{r data-structure-rows, exercise=TRUE, exercise.cap="Run and observe 8"}
nrow(unFBposts)
```

Each row is one Facebook post, so this tells you how many posts the UN published in 2015. You will use `nrow()` regularly this semester to confirm that a data set loaded correctly and to know how many observations you are working with.

**Practice 9: Calculate average posts per day**

Using what you just learned about the number of rows, calculate how many posts were published on average per day in 2015. Assume 365 days in the year. Three hints are available. Click "Submit Answer" to check your work.

```{r data-howmanyposts, exercise=TRUE, exercise.cap="Practice 9"}
...
**Practice 11: Display the variable names in unFBposts**

Display the names of the variables in **unFBposts**. Three hints are available. Click "Submit Answer" to check your work.

```{r data-explore-names, exercise=TRUE, exercise.cap="Practice 11"}
...
**Run and observe 12: Display the first few rows**

```{r data-explore-head1, exercise=TRUE, exercise.cap="Run and observe 12"}
head(unFBposts, n = 5)
```
...
**Run and observe 13: Display the structure of unFBposts**

```{r data-explore-head2, exercise=TRUE, exercise.cap="Run and observe 13"}
str(unFBposts)
```
...
**Practice 14: Read a variable type from `str()` output**
...
**Run and observe 15: Summarize unFBposts**

```{r data-explore-summary, exercise=TRUE, exercise.cap="Run and observe 15"}
summary(unFBposts)
```
...
**Practice 16: Interpret `summary()` output**
...
**Run and observe 17: Access a variable using $**

```{r data-explore-type, exercise=TRUE, exercise.cap="Run and observe 17"}
unFBposts$type
```

Here we can see that the variable **type** records whether a post contains a note, link, photo, event, or video.

**Practice 18: Access the likes_count variable**
...
```{r data-explore-dollar, exercise=TRUE, exercise.cap="Practice 18"}
...
**Practice 19: Retrieve the first post**
...
```{r text, exercise=TRUE, exercise.cap="Practice 19"}
...
**Run and observe 20: Check for missing values**

```{r data-explore-NAvalue, exercise=TRUE, exercise.cap="Run and observe 20"}
...
**Practice 21: Count missing values in the message variable**

Count how many posts are missing a value for **message**. Three hints are available. Click "Submit Answer" to check your work.

```{r data-handle-summation, exercise=TRUE, exercise.cap="Practice 21"}
...
**Practice 22: Count missing values in likes_count**

Checking for missing values is standard practice at the start of any analysis --- even when you expect none. A variable that appears complete may have `NA` values introduced during data cleaning or merging that you did not anticipate. Count the missing values in **likes_count** to confirm. Three hints are available. Click "Submit Answer" to check your work.

```{r data-handle-summation2, exercise=TRUE, exercise.cap="Practice 22"}
...
**Run and observe 23: See the NOT operator in action**

```{r data-not-operator, exercise=TRUE, exercise.cap="Run and observe 23"}
!TRUE
!FALSE
```
...
**Practice 24: Count non-missing values in likes_count**

Count how many posts have a recorded value for **likes_count**. Three hints are available. Click "Submit Answer" to check your work.

```{r data-handle-summation3, exercise=TRUE, exercise.cap="Practice 24"}
...
**Run and observe 25: Calculate the mean with na.rm = TRUE**

```{r data-handle-NAvalue, exercise=TRUE, exercise.cap="Run and observe 25"}
...
**Practice 26: Mean likes**

How many likes (**likes_count**) did posts receive on average? Include `na.rm = TRUE`. Three hints are available. Click "Submit Answer" to check your work.

```{r likes, exercise=TRUE, exercise.cap="Practice 26"}
...
grade_this({
  ...
  if (!grepl("unFBposts\\s*\\$\\s*likes_count", code_no_comments)) {
    fail("Make sure the function is applied to unFBposts$likes_count.")
  }
...
**Practice 27: Maximum shares**

What was the largest number of shares (**shares_count**) any post received? Include `na.rm = TRUE`. Three hints are available. Click "Submit Answer" to check your work.

```{r practice-max-shares, exercise=TRUE, exercise.cap="Practice 27"}
...
grade_this({
  ...
  if (!grepl("max\\s*\\(", code_no_comments)) {
    fail("You need the function that returns the largest value in a vector.")
  }
  if (!grepl("unFBposts\\s*\\$\\s*shares_count", code_no_comments)) {
    fail("Make sure the function is applied to unFBposts$shares_count.")
  }
  if (!grepl("na\\.rm\\s*=\\s*TRUE", code_no_comments)) {
...
**Practice 28: Minimum likes**

What was the smallest number of likes (**likes_count**) any post received? Include `na.rm = TRUE`. Three hints are available. Click "Submit Answer" to check your work.

```{r practice-min-likes, exercise=TRUE, exercise.cap="Practice 28"}
...
grade_this({
  ...
  if (!grepl("min\\s*\\(", code_no_comments)) {
    fail("You need the function that returns the smallest value in a vector.")
  }
  if (!grepl("unFBposts\\s*\\$\\s*likes_count", code_no_comments)) {
    fail("Make sure the function is applied to unFBposts$likes_count.")
  }
...
**Practice 29: Range of shares**

What was the range of shares (**shares_count**) across all posts? Include `na.rm = TRUE`. Remember that `range()` returns two values --- the minimum and maximum --- not a single number. Three hints are available. Click "Submit Answer" to check your work.

```{r practice-range-shares, exercise=TRUE, exercise.cap="Practice 29"}
...
**Run and observe 30: Compare two variables**

```{r comparison-operators, exercise=TRUE, exercise.cap="Run and observe 30"}
...
**Run and observe 32: Access a range of rows from a named column**

```{r data-explore-index2, exercise=TRUE, exercise.cap="Run and observe 32"}
...
**Run and observe 33: Find the most-liked message**

```{r data-explore-index3, exercise=TRUE, exercise.cap="Run and observe 33"}
unFBposts$message[which(unFBposts$likes_count == max(unFBposts$likes_count, na.rm = TRUE))]
```
...
**Practice 34: Find the most-shared message**

Now find the text of the message that received the most shares. Use **shares_count** to identify the row and **message** to retrieve the text. Three hints are available. Click "Submit Answer" to check your work.

```{r data-explore-index4_1, exercise=TRUE, exercise.cap="Practice 34"}
...
**Run and observe 35: Find the maximum shares value**

```{r data-explore-index4_2, exercise=TRUE, exercise.cap="Run and observe 35"}
unFBposts$shares_count[which(unFBposts$shares_count == max(unFBposts$shares_count, na.rm = TRUE))]
```

**Practice 36: Find the likes_count for the most-shared post**

Using the same row-selection logic, find the number of likes that the most-shared post received. This time retrieve **likes_count** instead of **shares_count**. Three hints are available. Click "Submit Answer" to check your work.

```{r data-explore-index4_3, exercise=TRUE, exercise.cap="Practice 36"}
...
**Run and observe 37: Create a new variable**
  
```{r data-create-variable1, exercise=TRUE, exercise.setup="prepare-like-or-share", exercise.cap="Run and observe 37"}
unFBposts$like.or.share <- unFBposts$likes_count + unFBposts$shares_count
```

We can check the first few values of the new variable using `head()`.

**Run and observe 38: Inspect the new variable**
  
```{r data-create-variable2, exercise=TRUE, exercise.setup="prepare-like-or-share", exercise.cap="Run and observe 38"}
...
**Practice 39: Create an attention variable**
...
```{r data-create-variable, exercise=TRUE, exercise.cap="Practice 39"}
...
**Apply 40: Count the posts with the fewest likes**

How many posts tied for the fewest likes of any post in the data set? Include `na.rm = TRUE`. Three hints are available. Click "Submit Answer" to check your work.

```{r apply-least-liked, exercise=TRUE, exercise.cap="Apply 40"}
...
# likes_count takes, and a count of how many posts
...
# Hint 2 of 3
# Comparing likes_count to its own smallest value
...
grade_this({
  correct <- sum(unFBposts$likes_count == min(unFBposts$likes_count, na.rm = TRUE), na.rm = TRUE)
  fewest  <- min(unFBposts$likes_count, na.rm = TRUE)
  ...
  if (!grepl("min\\s*\\(", code_no_comments)) {
    fail("You need the smallest value of likes_count before you can count how many posts match it.")
  }
  if (!grepl("unFBposts\\s*\\$\\s*likes_count", code_no_comments)) {
    fail("Use likes_count --- that is the variable holding the number of likes.")
  }
  if (!grepl("na\\.rm\\s*=\\s*TRUE", code_no_comments)) {
  ...
  if (!isTRUE(all.equal(as.numeric(.result), as.numeric(correct)))) {
    fail("Check that you are comparing likes_count to its own smallest value.")
  }
  pass(paste0("Exactly --- ", correct, " posts tied at ", fewest,
              " likes. A tie that large is worth pausing over. Every one of those posts is missing its message text: they are posts with nothing written in them, which is very likely why nobody engaged with them. Noticing patterns like this in the data is part of the work."))
})
```

**Apply 41: Find the month of the most-commented post**

In which month (**month**) was the most-commented post (**comments_count**) published? Retrieve the **month** value for the post with the maximum **comments_count**. Include `na.rm = TRUE`. Three hints are available. Click "Submit Answer" to check your work.

```{r apply-month-comments, exercise=TRUE, exercise.cap="Apply 41"}
...
**Apply 42: Create a variable and find its maximum**

Create a new variable called `full.engagement` in **unFBposts** that sums **likes_count**, **shares_count**, and **comments_count**. Then find the **message** text of the post with the highest total engagement. Include `na.rm = TRUE` in your `max()` call. Three hints are available. Click "Submit Answer" to check your work.

```{r apply-total-engagement, exercise=TRUE, exercise.cap="Apply 42"}
...
**Apply 43: Count posts where shares exceed likes**

How many posts received more shares than likes? Combine `>` with `sum()` to count them, and include `na.rm = TRUE`. Four hints are available. Click "Submit Answer" to check your work.

```{r apply-shares-exceed-likes, exercise=TRUE, exercise.cap="Apply 43"}
...
**Apply 44: Diagnose a failed load**
...
question("A student runs `counties <- load(\"counties2016.RData\")` and then runs `nrow(counties)`, which returns `NULL` instead of a number. What went wrong?",
  ...
  answer("`load()` creates the object automatically and returns only the name of what it created. Assigning that result replaced the data frame with a character string. The fix is to run `load(\"counties2016.RData\")` on its own line, then refer to **counties**.", correct = TRUE,
         message = "This is the single most common early mistake with these two functions, and it is worth understanding exactly what happened. `load()` did its job: it created a data frame called **counties**. But it also returns the *name* of what it created, as text. The assignment then overwrote the freshly created data frame with that piece of text --- so the data was created and destroyed in the same line. `nrow()` returns `NULL` because a character string has no rows. Remember the rule as: `load()` you run, `import()` you assign."),
...
**Apply 45: Diagnose a retrieval that returns the wrong thing**
...
question("A student wants to know the month of the most-shared post and writes `unFBposts$shares_count[which(unFBposts$shares_count == max(unFBposts$shares_count, na.rm = TRUE))]`. The code runs and returns a number. Why is that number not the answer?",
  ...
  answer("The variable in front of the square brackets is **shares_count**, so the expression returns the largest share count itself. The variable retrieved should be **month**; the one inside `which()` stays **shares_count**.", correct = TRUE,
  ...
  answer("The comparison should use `>` rather than `==`.",
         message = "`>` would return every row above some threshold rather than the single row holding the maximum. `==` is correct here: it matches the row whose value equals the maximum."),
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Incorrect. Try again."
)
...
**Apply 46: Evaluate a claim about a data set**
...
question("A student loads a new data set, runs `ncol()`, gets 15, and writes: \"This data set contains 15 observations.\" What is wrong with that sentence?",
  ...
  answer("`ncol()` counts columns, and columns are variables. Observations are rows, counted with `nrow()`. The data set has 15 variables, and the number of observations is still unknown.", correct = TRUE,
         message = "Rows are cases and columns are measurements --- that mapping underlies everything else in this course. A data set of 15 variables might have 50 observations or 50,000, and `ncol()` says nothing about which. It is worth running both functions whenever you open a new file: `nrow()` tells you how many cases you have to work with, and `ncol()` tells you how many measurements are available on each one."),
