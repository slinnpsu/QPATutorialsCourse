...
grade_this({
  ...
  pass("`names()` returns the variable names in the order they sit in the data frame --- the labels you use with `$` to reach any one of them.")
})
```
...
grade_this({
  ...
  pass("The `$` operator extracted the entire column as a vector --- that long list of numbers is every **likes_count** value in the data set, one per row.")
})
```
