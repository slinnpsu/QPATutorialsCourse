...
In Tutorial 1 all of our data was entered by hand. In practice, data almost always arrives as a file created by someone else --- a spreadsheet, a statistical software file, or a plain text file. Regardless of format, data sets share a common structure: each row contains one [observation]{.important-text} (all the values recorded for a single case) and each column contains one [variable]{.important-text} (all the values of a single measurement across all cases). This is the same structure you would see in a spreadsheet. Once you start seeing data this way --- observations in rows, variables in columns --- many R commands become easier to understand.
...
grade_this({
  ...
  if (!inherits(.result, "data.frame"))
    fail("oil should be a data frame. Make sure you are using import() to read oilwomen.dta.")
...
grade_this({
  ...
  if (!inherits(.result, "data.frame")) {
    fail("TO should be a data frame. Make sure you are importing turnout2.txt.")
  }
...
```{r likes-hint-3}
# Hint 3 of 3: fill in the blank
___(unFBposts$likes_count, na.rm = TRUE)
```
...
# Hint 3 of 3: fill in the blank
___(unFBposts$shares_count, na.rm = TRUE)
```
...
# Hint 3 of 3: fill in the blanks
unFBposts$message[which(unFBposts$___ == max(unFBposts$___, na.rm = TRUE))]
```
...
# Hint 3 of 3: fill in the blank
unFBposts$___[which(unFBposts$shares_count == max(unFBposts$shares_count, na.rm = TRUE))]
```
...
Which post received the fewest likes? Retrieve the **message** text of the post with the minimum **likes_count**. Include `na.rm = TRUE`. Click "Submit Answer" when ready. Four hints are available.
...
```{r apply-least-liked-hint-4}
# Hint 4 of 4: fill in the blanks
unFBposts$variable.to.retrieve[which(unFBposts$selection.variable == min(unFBposts$selection.variable, na.rm = TRUE))]
```
...
In which month (**month**) was the most-commented post (**comments_count**) published? Retrieve the **month** value for the post with the maximum **comments_count**. Include `na.rm = TRUE`. Click "Submit Answer" when ready. Four hints are available.
...
```{r apply-month-comments-hint-4}
# Hint 4 of 4: fill in the blanks
unFBposts$___[which(unFBposts$comments_count == max(unFBposts$___, na.rm = TRUE))]
```
...
Create a new variable called `full.engagement` in **unFBposts** that sums **likes_count**, **shares_count**, and **comments_count**. Then find the **message** text of the post with the highest total engagement. Include `na.rm = TRUE` in your `max()` call. Click "Submit Answer" when ready. Four hints are available.
...
# Hint 1 of 4
# This exercise has two parts: first create the
# new variable, then use it to find a post.
...
# Part 2 structure:
# unFBposts$message[which(unFBposts$full.engagement == max(...))]
```

```{r apply-total-engagement-hint-4}
# Hint 4 of 4: fill in the blanks
unFBposts$full.engagement <- unFBposts$likes_count + unFBposts$___ + unFBposts$___
unFBposts$message[which(unFBposts$full.engagement == max(unFBposts$___, na.rm = TRUE))]
```
...
How you **load data** depends on the file type, and the distinction is worth committing to memory --- as the caution box above the "Learning about the data" section notes, mixing up the two approaches is one of the most common early mistakes. R data files (.RData) are loaded with `load()`, which creates objects automatically; all other formats are imported with `import()` from the [rio]{.package-name} package, and you must assign the result to an object.
...
- `nrow()` and `ncol()` --- tells you how many observations and variables you have
