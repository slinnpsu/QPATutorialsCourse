...
Data rarely arrive holding the variable your question is about. A file gives you what was collected: counts, dates, categories, whatever the source chose to record. A research question is usually about something built from those --- a total, a rate, a share, a difference between two columns. If your question is about how much engagement a post drew, neither likes nor shares answers it on its own. Creating a variable is how you close that gap.

Creating it *inside* the data frame matters too. A variable stored there stays lined up with every other variable, row by row, so it travels with the data when you filter, group, summarize or plot. A standalone object holding the same numbers would have to be kept in step by hand.

You can create a new variable inside an existing data frame using the `$` operator combined with `<-`. The syntax is `data_frame$new_variable <- expression`. The expression on the right side can combine any variables already in the data frame, as long as each one is also referenced with `$`.
