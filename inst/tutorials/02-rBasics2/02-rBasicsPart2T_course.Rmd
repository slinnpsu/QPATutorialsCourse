...
::: {.caution}
**Key distinction:** Unlike `load()`, which creates objects automatically, `import()` returns a data frame that you must assign to an object yourself. Mixing these up is one of the most common early mistakes.
:::
...
This check confirms that the new variable was created and lets you inspect its first few values. It shows only the top of the vector, not every row --- if you wanted to know that R produced a value for each of the 1,643 posts, `length()` would tell you.
...
How many posts received more shares (**shares_count**) than likes (**likes_count**)? Combine `>` with `sum()` to count them. Neither variable has missing values, so no `na.rm = TRUE` is needed. Two hints are available. Click "Submit Answer" to check your work.
