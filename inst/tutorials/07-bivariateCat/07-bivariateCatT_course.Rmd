...
The `x` argument defines the variable that appears in the rows, so we use it for the outcome variable. The `y` argument defines the variable that appears in the columns, so we use it for the explanatory variable. By default, `CrossTable()` reports row proportions, column proportions, and total proportions all at once, which makes the table harder to read. Set `prop.c = TRUE` to keep column proportions and `prop.r = FALSE`, `prop.t = FALSE`, and `prop.chisq = FALSE` to suppress everything else.
...
question("Which of the following statements are true? Select all that apply.",
  ...
  answer("Presidential democracies have the highest proportion of countries with medium gender equality",
         message = "That claim reads down the Presidential democracy column to find its largest category. The table does not support it, and it is a different kind of statement from the one above: reading down a column tells you how one regime type is distributed, while reading across a row compares regime types to each other."),
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Two of the three are true. For each one, ask whether the proportion quoted is a share of its column or of something else."
)
```
...
```{r barplot-dodge-trump-majority-rural, exercise=TRUE, exercise.cap="Run and observe 11", fig.alt="Grouped bar chart of US counties. The x-axis has two positions, one for counties where Clinton won a majority and one for counties where Trump won, and within each position three bars of different colors stand side by side for the three rurality categories. The y-axis is labeled count. The three bars in the Trump group are all far taller than the three in the Clinton group."}
library(ggplot2)
library(dplyr)
counties %>%
...
```{r barplot-proportions-trump-majority-rural, exercise=TRUE, exercise.cap="Run and observe 12", fig.alt="The same grouped bar chart, now with a y-axis running from 0 to 1 and labeled prop. Within each rurality color, the Clinton bar and the Trump bar together reach 1. The Trump bars rise from about 0.74 for urban counties to about 0.93 for rural counties, and the Clinton bars fall correspondingly."}
counties %>%
  filter(!is.na(TrumpMajority), !is.na(rural)) %>%
  ggplot(mapping = aes(x = factor(TrumpMajority), fill = factor(rural))) +
...
# outcome. Which variable is which here?
```

```{r exercise-create-proportions-vote-race-majority-hint-2}
# Hint 2 of 4
# First drop the counties missing on either variable.
# filter() takes one condition per variable, separated by a
# comma, and each condition negates is.na() for that
# variable. Both variables need one.
```
...
# variable you mapped to fill. Without that group, the
# proportions are computed over all counties rather than
# within each racial majority category.
```
...
grade_this({
  ...
  if (!grepl("!\\s*is\\.na\\s*\\(\\s*racial_majority\\s*\\)", code_no_comments)) {
    fail("Filter out missing values for **racial_majority** inside `filter()`.")
  }
  if (!grepl("!\\s*is\\.na\\s*\\(\\s*TrumpMajority\\s*\\)", code_no_comments)) {
    fail("Filter out missing values for **TrumpMajority** inside `filter()`.")
  }
  ...
  if (!grepl("TrumpMajority", x_expr)) {
    fail("Map **TrumpMajority** to the x-axis inside `aes()`.")
  }
...
```{r exercise-create-proportions-gender-equality-regime, exercise=TRUE, exercise.cap="Practice 17", fig.alt="Grouped proportion bar chart titled Gender Equality, with three x-axis positions for low, medium, and high gender equality and, within each, three bars for the regime types named in the legend: Dictatorship, Parliamentary Democracy, and Presidential Democracy. The y-axis is labeled Proportion. At the high end of the scale the Dictatorship bar is shorter than either democracy bar."}
world %>%
...
# Hint 2 of 4
# The filter() step is already written --- continue the
# pipeline into ggplot(). Map gender_equal3 to x and
# regime_type3 to fill in the main aes(). Both are stored as
# factors, so neither needs factor().
```
...
# regime_type3. Then labs() carries the title, the y-axis
# label, and the caption, with x set to NULL and no subtitle
# at all.
```
...
# order that levels(world$regime_type3) returned in Run and
# observe 16 --- in the wrong order, the wrong label sits
# beside each bar color.
```
