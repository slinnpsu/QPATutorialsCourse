...
The hypothesis predicts that the proportion of counties where Trump won a majority should increase as counties become more rural. You would also expect the reverse pattern in the Clinton-majority row: the proportion of Clinton-majority counties should decrease as counties become more rural.

::: {.caution}
**Cross-tab analysis is not a hypothesis test.** A cross-tab can show whether the observed pattern is *consistent with* a hypothesis. It does not tell you whether the difference is larger than you would expect by chance. Formal hypothesis tests are covered in later tutorials.
:::
...
The data frame **counties** contains a nominal variable called **racial_majority** that takes the character values "Black Majority," "Hispanic Majority," "Other/No Majority," and "White Majority." Like **TrumpMajority**, it has no missing values, so again all 3,112 counties are included. This example uses **racial_majority** as the explanatory variable and **TrumpMajority** as the outcome variable.
...
Remember, this is a descriptive comparison. The cross-tab can show whether the observed pattern is consistent with the hypothesis, but it does not explain why the pattern exists or tell you whether the relationship is statistically significant.
...
**`position = "dodge"`** inside `geom_bar()` places the fill categories side by side within each x-axis group rather than stacking them. Stacked bars show how a total is composed; side-by-side bars make it easy to compare heights across fill categories, which is what you need to evaluate a hypothesis.
...
Now the y-axis shows proportions rather than counts. For each rurality category, the Clinton and Trump proportions add to 1. This lets you compare the share of counties in each rurality category that gave a majority of votes to Clinton or Trump. The plot shows that the proportion of Trump-majority counties is highest among rural counties and lowest among urban counties.
...
grade_this({
  ...
  if (length(x_scales) == 0 ||
      !identical(x_scales[[1]]$labels[names(expected_x)], expected_x)) {
  ...
  if (length(fill_scales) == 0 ||
      !identical(fill_scales[[1]]$labels[names(expected_fill)], expected_fill)) {
    fail('Pair each stored value with its legend label, as in "Black Majority" = "Black". All four racial majority categories need a label.')
  }
...
The output shows the stored values, which are what you pair each replacement label with in `scale_fill_discrete()`.
...
grade_this({
  ...
  fill_scales <- Filter(function(sc) "fill" %in% sc$aesthetics, .result$scales$scales)
  fill_labels <- if (length(fill_scales) > 0) fill_scales[[1]]$labels else NULL
  expected_labels <- c("Dictatorship" = "Dictatorship",
                       "Parliamentary democ" = "Parliamentary\nDemocracy",
                       "Presidential democ" = "Presidential\nDemocracy")

  if (!identical(fill_labels[names(expected_labels)], expected_labels)) {
    fail('Pair each stored value with its legend label, as in "Parliamentary democ" = "Parliamentary\\nDemocracy". All three categories need a label, and the two democracy labels need their \\n line breaks.')
  }
...
The completed plot presents the same information as the gender-equality cross-tab, but in visual form. The plot shows that dictatorships have a lower proportion of countries with high gender equality than either democratic regime type, consistent with the hypothesis.
...
The cross-tab and the grouped bar plot are complementary: the table gives exact numbers, the plot makes the pattern easier to see at a glance. For evaluating a hypothesis, you usually want both. A cross-tab can show whether the observed pattern is *consistent with* a hypothesis, but it does not tell you whether the difference is larger than you would expect by chance --- that requires a formal hypothesis test, which Tutorial 12 takes up for two categorical variables.
