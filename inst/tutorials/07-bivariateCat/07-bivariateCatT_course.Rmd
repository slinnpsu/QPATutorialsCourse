...
The cross-tab and the grouped bar plot are complementary: the table gives exact numbers, the plot makes the pattern easier to see at a glance. For evaluating a hypothesis, you usually want both. A cross-tab can show whether the observed pattern is *consistent with* a hypothesis, but it does not tell you whether a pattern this large would be surprising if there were no systematic relationship between the two variables --- that requires a formal hypothesis test, which Tutorial 12 takes up for two categorical variables.
