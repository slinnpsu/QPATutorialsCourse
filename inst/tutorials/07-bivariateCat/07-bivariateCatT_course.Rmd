...
This tutorial covers the case where both variables are categorical --- both nominal, both ordinal, or one of each. The tools are the cross-tabulation and the grouped bar plot. Each section opens with a research question and a hypothesis, builds the table, and asks whether the pattern in it matches what the hypothesis predicts. If you need to settle whether a variable counts as nominal or ordinal, Tutorial 3 is where that distinction is drawn.
...
grade_this({
  ...
  if (!identical(plot_labels$caption, "Cheibub Democracy and Dictatorship dataset; World Values Survey")) {
    fail('Add `caption = "Cheibub Democracy and Dictatorship dataset; World Values Survey"` inside `labs()`.')
  }
