...
2. $\hat{\beta}_2 + \hat{\beta}_3$ is the slope of $X_2$ [**for the non-reference category**]{.important-text}. This is the step most often missed: the coefficient on $X_2$ gives the reference-category slope; you must add the interaction coefficient to get the non-reference-category slope.
...
question("The interaction coefficient in the row labeled **mortality_risk_25_45**:**wage_growth** is -0.139. Which statement correctly describes what this number means?",
  ...
  answer("For each additional percentage point of wage growth, the estimated slope of mortality risk on Clinton's vote becomes 0.139 percentage points more negative",
         correct = TRUE,
         message = "Because the mortality-risk slope is negative when wage growth equals zero, becoming more negative means that higher mortality risk predicts a larger drop in Clinton support as wage growth increases. The next section will let you compute exactly how large that slope is at specific values of wage growth."),
...
## Does the History–Representation Relationship Depend on Electoral System?
...
The data frame **ipu** contains **women_percent**, the percentage of seats held by women, and **electoral_f**, the three-category factor from Tutorial 16 with **Plurality/Majority** as the reference category. **quota_f** indicates whether the country has an electoral gender quota.
...
```{r exercise-ipu-models, exercise=TRUE, exercise.cap="Run and observe: Estimate the additive and interaction models for history and electoral system"}
model_rep_additive <- lm(women_percent ~ quota_f + electoral_f + years_c, data = ipu)
model_rep_int <- lm(women_percent ~ quota_f + electoral_f * years_c, data = ipu)
summary(model_rep_int)
```
...
**Practice: Identify the three history slopes**
...
question("Using the coefficients in your output, which statement correctly identifies the three conditional slopes of years_c?",
  ...
  answer("Plurality: the coefficient on years_c directly; Proportional: the years_c coefficient + the Proportional interaction coefficient; Mixed: the years_c coefficient + the Mixed interaction coefficient",
         correct = TRUE,
...
**Apply: Interpret the interaction coefficients**
...
question("Your output gives a years_c coefficient of 0.194, a Proportional interaction coefficient of -0.102, and a Mixed interaction coefficient of -0.232. Which statement correctly describes the three conditional slopes and what they mean?",
  ...
  answer("Plurality: 0.194; Proportional: 0.194 + (-0.102) = 0.091; Mixed: 0.194 + (-0.232) = -0.038. Under plurality rules, for each year earlier that a parliament first seated a woman, it is predicted to seat about 0.19 percentage points more women today. Under proportional rules that figure is about 0.09. Under mixed rules the sign flips: the earlier the first woman arrived, the slightly fewer women are predicted now.",
         correct = TRUE,
         message = "History matters roughly twice as much under plurality rules as under proportional ones, which is the direction the institutional argument predicted. The Mixed slope is the surprise --- it is close to zero, and it points the other way."),
...
::: {.caution}
**A note on the threshold.** An interaction term is estimated from differences between subgroups, so it carries less information than a main effect and is correspondingly harder to detect. Here the subgroups are 65 plurality systems, 77 proportional, and 36 mixed --- and the smallest of those carries the comparison the test depends on most. We will therefore evaluate this interaction at $\alpha = .10$ rather than the $\alpha = .05$ used elsewhere in the course. That choice is made here, before the test is run, and for the reason just given. Tutorial 11 explains why .05 is a convention rather than a rule.
:::

**Run and observe: Joint F-test for the history × electoral system interaction**

```{r exercise-ipu-anova, exercise=TRUE, exercise.cap="Run and observe: Joint F-test comparing additive and interaction models"}
anova(model_rep_additive, model_rep_int)
```

The F-statistic tests whether the two interaction terms together add significant explanatory power over the additive model.

### Assess the Evidence for the Interaction

**Apply: Evaluate whether the interaction is supported**
...
question("The joint F-test gives F = 2.49 with p = .086. The Proportional interaction coefficient has p = .224 and the Mixed interaction coefficient has p = .027. Which statement best describes what the data tell us?",
  correct = "Right. The interaction is supported at the .10 level, but the contrast the hypothesis is about is not the one the data distinguish.",
  answer("The joint test supports the interaction at the .10 threshold set for this model. But the two interaction terms behave differently: the Mixed comparison is statistically distinguishable from the Plurality slope (p = .027), while the Proportional comparison is not (p = .224) --- and the Proportional comparison is the one the hypothesis is about.",
         correct = TRUE,
         message = "This is the honest reading. The overall pattern runs the way the institutional argument predicts, and the joint test supports treating the slopes as unequal. But the specific claim --- that history matters more under plurality rules than under proportional ones --- is the comparison with the weakest evidence. The strong evidence concerns mixed systems, which the argument says little about."),
  answer("The interaction is confirmed, and the hypothesis is supported, because the joint F-test clears the .10 threshold",
         message = "The joint test tells us the slopes are not all equal. It does not tell us which pair differs. Here the Proportional-versus-Plurality contrast --- the one the hypothesis names --- has p = .224, so the joint result is being carried by a comparison the hypothesis did not make."),
  answer("The interaction should be rejected because the joint test does not clear .05",
         message = "The threshold for this model was set at .10 before the test was run, and for a stated reason: interaction terms are estimated from subgroup differences and are harder to detect. Moving the goalposts after seeing the p-value would be the problem; choosing them in advance is not."),
...
::: {.research-example}
The interaction is supported at the .10 level: the joint test gives $F = 2.49$, $p = .086$, and the model's $R^2$ rises from .307 to .327. The three conditional slopes tell the story the coefficients only imply. Under plurality rules, for each year earlier that a parliament first seated a woman, it is predicted to seat 0.194 percentage points more women today. Under proportional rules that figure is 0.091 --- less than half. Under mixed rules it is -0.038, essentially nothing.

The institutional argument anticipated the first two. What it did not anticipate is that the only contrast the data distinguish clearly is the one it says least about. Plurality against proportional --- the comparison the theory is built on --- gives $p = .224$. Plurality against mixed gives $p = .027$. So the pattern points the way the theory predicts while the evidence for the specific claim is weak, and the evidence that *is* strong concerns a category the theory treats as a residual.

The crossing points make this concrete, once translated. Zero on the centered scale is a parliament that first seated a woman around 1966. The mixed and plurality lines cross at a parliament whose first woman arrived around 1941. Where the first woman arrived *later* than that, mixed systems are predicted to seat more women than plurality systems; where she arrived *earlier*, plurality systems are predicted to seat more --- and the data run well back past 1941, to first women arriving in 1907. The proportional and plurality lines would cross at a parliament whose first woman arrived around 1895, earlier than any country in the file. So across the whole observed range, proportional systems stay ahead; it is only the mixed comparison that actually reverses.

The two lines are not converging on a common value. Under plurality rules, the longer ago a parliament seated its first woman, the more women it is predicted to seat today. Under mixed rules the prediction runs the other way: the longer ago the first woman arrived, the *fewer* women are predicted now. The two are moving in opposite directions, which is what makes the crossing a reversal rather than a meeting point.

A model can support a hypothesis in its overall shape while leaving its central comparison unresolved, and saying so is more useful than reporting only the parts that worked.
:::
