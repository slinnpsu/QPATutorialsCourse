...
* How to test an interaction as a whole with a joint F-test comparing the additive and interaction models
...
**rights** is already coded 0 for countries with limited political rights and 1 for countries with broad political rights. Fill in the blanks to turn it into a labeled factor called **rights_f**, using the labels \"Few Rights\" and \"Many Rights\", then add a line displaying its levels so you can see which category R will treat as the reference. Three hints are available. Click "Submit Answer" to check your work.

```{r exercise-factor-rights, exercise=TRUE, exercise.cap="Practice 5"}
qog$rights_f <- ___(qog$rights, labels = c("___", "___"))
...
grade_this({
  ...
  code_nc <- gsub("#[^\r\n]*", "", code)
  if (!grepl("factor\\s*\\(", code_nc))
    fail("Use factor() to convert rights into a labeled factor.")
...
Write the call that compares the two models you just estimated. Two hints are available. Click "Submit Answer" to check your work.
...
grade_this({
  ...
  pos_add <- regexpr("model_rep_additive", code_nc)[1]
  pos_int <- regexpr("model_rep_int", code_nc)[1]
  if (pos_add > pos_int)
...
question("The joint F-test gives F = 2.49 with p = .086. The Proportional interaction coefficient has p = .224 and the Mixed interaction coefficient has p = .027. Which statement best describes what the data tell us?",
  ...
  answer("The joint test supports the interaction at .10, but only the Mixed comparison is distinguishable from the Plurality slope",
         correct = TRUE,
...
That last paragraph is worth noticing as a piece of writing. A model can support a hypothesis in its overall shape while leaving its central comparison unresolved, and saying so is more useful than reporting only the parts that worked.
