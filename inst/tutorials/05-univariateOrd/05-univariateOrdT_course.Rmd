...
Note that the median depends on how the categories are ordered and coded, so it is essential to state how the scale runs whenever you report it. Here, category 1 is the most urban and category 9 is the most rural, so a median of 6 places the midpoint of the county distribution toward the more rural end of the scale. The **FHStatus** example will show what happens when the mode and median do not agree.
...
You built frequency bar charts in Tutorial 4 using `ggplot()`, `geom_bar()`, `labs()`, and `scale_x_discrete()` --- the same structure applies here. The one addition is `coord_flip()` as a final layer, which flips the bars horizontal and makes the nine category labels readable. **rural_urban** has no missing values, so no filtering is needed here. Remember that it is stored as a numeric variable, even though its nine values are ordered categories rather than quantities. Complete the code below: map the variable to the x-axis, add the bar layer with its fill colour, and add the final layer that flips the bars. The plot labels and the nine shortened category labels are already supplied. Set the bar fill to \"firebrick\". Three hints are available. Click \"Submit Answer\" to check your work.
...
# Hint 3 of 3
# The fill colour is a fixed setting applied to every bar,
...
grade_this({
  ...
  pass("Well done! Your **FHStatus** table is complete, and the eight countries with no recorded status were excluded before the proportions were computed --- so each proportion is a share of the countries that have a rating.")
})
```
...
Now ask whether the midpoint of the ordered distribution falls in the same category as the mode. Because **FHStatus** is coded in rank order --- 0 for Not Free through 2 for Free --- you can calculate its median directly.

**Practice 9: Calculate the median for FHStatus**

Use `median()` with `na.rm = TRUE` to calculate the median of **FHStatus** in the **fHouse** data frame. Three hints are available. Click "Submit Answer" to check your work.
...
Build a proportion bar chart for **FHStatus**. Filter out the missing values before piping the data into `ggplot()`, wrap the variable in `factor()` so the codes are treated as categories, and use the proportion mapping inside `geom_bar()` that you saw earlier in this tutorial. As in Tutorial 4, the data arrive via a pipe, so only `mapping =` is needed, not `data =`. In `scale_x_discrete()`, label the categories in order from code 0 to 2: \"Not Free\", \"Partly Free\", \"Free\".

Title: \"Freedom House Ranking of Countries\", y-axis: \"Proportion\", caption: \"Source: Freedom House\", fill: \"steelblue\". Three hints are available. Click \"Submit Answer\" to check your work.

```{r visual-practice-fhstatus-prop-bar, exercise=TRUE, exercise.cap="Practice 10"}
