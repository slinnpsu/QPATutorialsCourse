...
# proportions. The same sequence continues here.
...
# Hint 3 of 3
# The pipeline has three steps after counties:
# group_by() takes the variable name,
# summarise() creates freq using n(),
# and mutate() creates prop by dividing freq
# by the sum of all freq values.
# The column names freq and prop are specified
# on the left side of the = in each function.
```
...
question("You just computed the mode and median of rural_urban. What does it mean when the mode and median fall in the same category?",
  ...
  answer("The most common county type and the middle county type are the same --- both have an urban population of 2,500 to 19,999 adjacent to a metro area", correct = TRUE,
         message = "When the mode and median agree, the most frequently occurring category is also the one that divides the distribution in half. For rural_urban, both fall in category 6. Two different summaries of central tendency therefore point to the same category, making the characterization of the typical US county especially clear."),
...
When the mode and median agree, two different ways of characterizing the center of the distribution point to the same category, making the typical value especially clear. That agreement does not always occur, as the FHStatus example will show. Note that the median depends on how the categories are ordered and coded, so it is essential to state how the scale runs whenever you report it. Here, category 1 is the most urban and category 9 is the most rural, so a median of 6 places the typical county toward the more rural end of the scale.
...
Build a complete frequency bar chart for **rural_urban**. Because this variable has nine categories with descriptive labels, use `scale_x_discrete()` to replace the numeric codes with shortened labels and `coord_flip()` as a final layer to flip the bars horizontal --- this makes long labels readable. Load [ggplot2]{.package-name} at the top with `library(ggplot2)` --- once loaded, you can assume it is available for all subsequent exercises in this tutorial. Four hints are available. Click "Submit Answer" to check your work.

Your chart should have:

- Title: "USDA Urban/Rural Classification of US Counties"
- Subtitle: "2016"
- y-axis label: "Frequency"
- x-axis label: omitted (`NULL`)
- Caption: "Source: Linn, Nagler, Zilinsky"
- Bar fill color: "firebrick"
- Category labels replacing the numeric codes, shortened from the codebook above (in the order the categories are stored in the data) --- type these yourself, do not copy and paste from this list:
  - 1 million+
...
# Hint 1 of 4
# In Tutorial 4 you built frequency bar plots with
# four layers: ggplot(), geom_bar(), labs(), and
# scale_x_discrete(). This plot needs a fifth layer:
# coord_flip() to turn the bars horizontal.
# What data frame and variable are you plotting?
```

```{r visual-practice-bar-rural-hint-2}
# Hint 2 of 4
# The five layers connected by + are:
# 1. ggplot(data = counties, mapping = aes(x = factor(rural_urban)))
#    -- sets up the plot and maps the variable to x
# 2. geom_bar(fill = "firebrick")
#    -- draws frequency bars in firebrick red
# 3. labs(title = "...", subtitle = "...", x = ..., y = "...", caption = "...")
#    -- adds title and axis labels
# 4. scale_x_discrete(labels = c(...))
#    -- replaces the numeric codes with shortened labels
# 5. coord_flip()
#    -- flips the bars horizontal
```
...
# the caption, and the nine shortened labels.
# rural_urban is stored as numeric, so wrap it in factor().
# Set x = NULL to suppress the x-axis title.
```

```{r visual-practice-bar-rural-hint-4}
# Hint 4 of 4
# Inside scale_x_discrete(), the labels argument
# takes a vector: labels = c("first", "second", ...).
# List all nine shortened labels in order from
# category 1 to 9 as given in the instructions.
# Add coord_flip() as the last layer.
```
...
grade_this({
  ...
  pass("Well done! With readable labels and horizontal bars, the shape of the distribution is much easier to interpret. Look at the relative bar lengths across all nine categories before reading the interpretation below.")
})
```

With readable labels and horizontal bars, the distribution is much easier to read. The bars are fairly similar in length across all nine categories, with no single category clearly standing out. This tells you three things:
...
Build a proportion bar chart for **FHStatus** from scratch. Pipe `fHouse` through `filter(!is.na(FHStatus))` before passing to `ggplot()`. Your chart should have:

- Title: "Freedom House Ranking of Countries"
- Subtitle: none --- the `subtitle` argument defaults to `NULL`, so you do not need to include it
- y-axis label: "Proportion"
- x-axis label: omitted (`NULL`)
- Caption: "Source: Freedom House"
- Bar fill color: "steelblue"
- Category labels replacing the numeric codes with the substantive category names

For the category labels, recall that replacement labels must be listed in the same order as the numeric values stored in the data, from lowest to highest. The three codes are 0, 1, and 2 --- look up what each represents in the variable description above. Four hints are available. Click "Submit Answer" to check your work.
...
# Hint 1 of 4
# A proportion bar chart needs the same layers
# as a frequency bar chart, with one addition
# inside geom_bar(): aes(y = after_stat(prop), group = 1).
# Start by piping fHouse through filter(!is.na(FHStatus))
# before passing to ggplot().
```

```{r visual-practice-fhstatus-prop-bar-hint-2}
# Hint 2 of 4
# The opening of the pipeline is:
# fHouse %>%
#   filter(!is.na(FHStatus)) %>%
#   ggplot(mapping = aes(x = factor(FHStatus))) +
# When data arrives via a pipe, ggplot() receives it
# automatically --- only mapping = is needed, not data =.
```

```{r visual-practice-fhstatus-prop-bar-hint-3}
# Hint 3 of 4
# Inside geom_bar(), add aes(y = after_stat(prop), group = 1)
# to transform bar heights from counts to proportions.
# after_stat(prop) computes each category's share of the whole;
# group = 1 ensures proportions are computed across all bars.
```

```{r visual-practice-fhstatus-prop-bar-hint-4}
# Hint 4 of 4
# The layers after the pipe are:
# geom_bar(aes(y = after_stat(prop), group = 1), fill = "steelblue"),
# labs() with the title, x = NULL, and y = "Proportion",
# and scale_x_discrete() with the three category labels
# in the order they are stored: Not Free, Partly Free, Free.
```
...
Apply the shape-decision routine from Tutorial 4 to describe the distribution of **FHStatus**.
...
question("Based on the proportion bar plot, how would you best describe the shape of the distribution of FHStatus?",
  correct = "Right. The distribution is somewhat unimodal but fairly dispersed.",
  ...
  answer("Somewhat unimodal --- Free is the most common category but the distribution is fairly dispersed across all three categories", correct = TRUE,
         message = "Free is the modal category, but Not Free and Partly Free are also common. The distribution has one peak, but it is spread enough that the full distribution is more informative than the mode alone. Not Free and Partly Free together account for more than half of all countries, which is why the median falls in Partly Free even though Free is the mode. When shape is not perfectly clean, reporting the full distribution is more informative than forcing a single label."),
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Incorrect. Try again."
)
...
question("A frequency table for a 4-category ordinal variable shows that the modal category is the highest value and the median falls in the second-highest category. What does this tell you about the distribution?",
  correct = "Right. The mode is above the median: the highest category is the most common, but enough cases fall below it that the midpoint lies in the second-highest category.",
  answer("The distribution is perfectly symmetric",
         message = "If the distribution were perfectly symmetric, the mode and median would coincide. Their divergence here shows they do not."),
  answer("The highest category is the most common response, but enough cases fall in lower categories that the median lies below the mode", correct = TRUE,
         message = "The mode being higher than the median means the most common value is at the top, but there are enough cases in the lower categories to shift the midpoint downward. This is the same pattern we saw with FHStatus: Free was the mode, but Partly Free was the median because Not Free and Partly Free countries together made up more than half of all cases. Note that the highest category being the mode does not mean most cases fall there --- it only needs more cases than any other single category. To describe the full shape of the distribution, you would need to see the frequencies across all four categories."),
