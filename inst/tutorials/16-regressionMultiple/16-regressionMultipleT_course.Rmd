...
question("Look at the slope on wage_growth in your output. Which statement correctly gives its value and what it means?",
  ...
  answer("The slope is 0.314; for every one-percentage-point increase in wage growth, Clinton's two-party vote percent is expected to rise by about 0.31 points",
         correct = TRUE,
...
question("What can you conclude from the hypothesis test on college_grad_percent?",
  ...
  answer("The p-value is far below .05 and the t-statistic is large, so we reject the null of no relationship, controlling for the other predictors",
         correct = TRUE,
...
The data frame **qog** contains country-level data from multiple sources, drawn from the January 2020 cross-section of the Quality of Government Basic Dataset. Variables in that cross-section take their most recent available observation as of that release, so they are not all measured in the same year --- consult the codebook with `?qog` when a specific year matters. Of the 194 countries, **wdi_fertility** is missing for 9, **wbgi_cce** for 2, **ipu_l_sw** for 1, and **fh_ipolity2** for none. Our outcome variable is **wbgi_cce**, the World Bank's control of corruption score --- it runs from about -2.5 to 2.5, where higher values indicate better control of corruption. Our predictors are:
...
In Tutorial 15 you built a factor with `factor()` and a `labels` argument, which named the categories but left R to order them. Here the order matters, so `factor()` takes a second argument: `levels`, a vector of the values **as they appear in the data**, in the order you want them. The first one becomes the reference category. `labels` then supplies the printed names, in that same order.
...
grade_this({
  ...
  pass("Correct. Plurality/Majority is the reference category, the cross-tab confirms each label landed on the right category, and years_c is centered at its mean of 60.3 years.")
})
```
...
question("In the multiple regression predicting Clinton's county vote share, the coefficient on wage growth is 0.314. Which interpretation is correct?",
  ...
  answer("A one-point rise in wage growth predicts a 0.314-point rise in Clinton's vote share",
         correct = TRUE,
  ...
  answer("Clinton's vote share causes wage growth to rise by 0.314 points in the average county",
