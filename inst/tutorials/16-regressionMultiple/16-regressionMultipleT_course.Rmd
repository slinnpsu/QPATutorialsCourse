...
[**Spurious relationships.**]{.important-text} Sometimes two variables appear to be related because a third variable causes both. If poor health contributes to weaker local economic conditions and also leads voters to oppose the incumbent party, wage growth and incumbent-party voting may appear to be related even if wage growth itself is not responsible for the voting difference. Including a health measure lets us estimate the wage-growth association among counties with similar measured health risk --- and if the coefficient shrinks, we have learned something important about what the original bivariate estimate was combining. A third variable that is related to both the predictor and the outcome in a way that distorts the estimated association is called a [**confounder**]{.important-text} --- and the distortion it produces is called [**confounding**]{.important-text}.

```{r diagram-spurious, echo=FALSE, out.width="70%", fig.align="center"}
knitr::include_graphics("images/spurious.png")
```

[**Multiple correlated causes.**]{.important-text} Most political and social outcomes have several potential causes, and the variables that represent them are often correlated with one another. Counties with higher college-educated populations tend to have higher wage growth and lower mortality risk. When an omitted variable is related to both an included predictor and the outcome, the included coefficient may combine their overlapping relationships rather than isolate the relationship of interest. This is called [**omitted variable bias**]{.important-text}, and it is the main reason you will almost never see a published regression with only one predictor.
...
::: {.caution}
**Controlling for variables reduces some alternative explanations but does not eliminate them.** Multiple regression holds measured variables constant, which is more informative than a simple bivariate association --- but it cannot guarantee that all relevant differences between cases have been accounted for. Variables not included in the model may still distort the relationships we estimate. Causal conclusions require a research design and assumptions that justify them; regression coefficients alone describe partial associations among the variables included in the model. Throughout this tutorial, we describe our estimates as relationships and associations, not causes.
:::
...
That qualifier matters more than it might seem. Without controls, the simple regression of Clinton's two-party vote percent on **wage_growth** alone yields a larger slope. In this model, controlling for mortality risk, education, and racial composition, the slope on **wage_growth** is smaller --- because some of the original wage-growth relationship overlapped with differences in education, mortality risk, and racial composition. The multiple-regression coefficient shows the remaining wage-growth relationship after those measured characteristics are held constant. Find the slope on **wage_growth** and see for yourself what controlling for the other variables does to it.
...
A slope is commonly called the [**effect**]{.important-text} of its predictor --- the expected change in the outcome for a one-unit increase in that predictor, holding the others constant. As with *predictor* and *predicts*, the word is used in this statistical sense and does not by itself say that one variable causes the other. The standard deviation is a natural anchor for gauging the size of an effect because it summarizes what a typical county-to-county difference looks like. A one-standard-deviation difference is common but not extreme. For any increase of size $c$, the predicted change in the outcome is $c \times \hat{\beta}$ --- the same multiplication you used to compute predicted values in Tutorial 15. This gives you a general recipe to follow whenever you want to assess the practical significance of regression coefficients: calculate the standard deviation of each predictor, choose $c$ as one standard deviation (or a round number close to it), and compute $c \times \hat{\beta}$ for each variable. Run the code below to get the standard deviations of the three focal variables.
...
The data frame **qog** contains country-level data from multiple sources. Our outcome variable is **wbgi_cce**, the World Bank's control of corruption score --- it runs from about -2.5 to 2.5, where higher values indicate better control of corruption. Our predictors are:

- **wdi_fertility**: the total fertility rate (average number of children per woman), standing in here for economic development
...
question("Look at both columns in the table above. Which statements are true? Select all that apply.",
  ...
  answer("In column (2) (Autocracy as reference), full democracies score about 0.68 points higher on control of corruption than autocracies on average, controlling for fertility and women's representation",
         correct = TRUE,
  ...
  answer("In column (2) (Autocracy as reference), the Partial Democracy coefficient is about 0.01 points and is not statistically significant --- partial democracies do not reliably control corruption better than autocracies once we control for fertility and women's representation",
         correct = TRUE,
  ...
  answer("The constant in column (2), 0.154, gives the predicted control of corruption score for autocracies when fertility rate and women's representation are both zero",
         correct = TRUE,
         message = "Correct. In column (2) the reference category is Autocracy, so the constant gives the predicted value for autocracies at X = 0. Compare to column (1): there the reference is Democracy, so the constant (0.835) gives the predicted value for full democracies at X = 0 --- a different quantity entirely."),
  ...
  answer("The choice of reference category changes the coefficients on wdi_fertility and ipu_l_sw",
...
One caution about the partial democracy estimate. Only 20 countries fall in that category, against 47 autocracies and 127 democracies, and small groups produce imprecise estimates --- the standard error on that coefficient is wide enough that a real difference of moderate size would be hard to detect. "Not statistically distinguishable" is not the same as "identical," and with 20 cases the distinction matters.
...
The output shows standard deviations of about 1.32 children per woman for **wdi_fertility** and about 12.2 percentage points for **ipu_l_sw**. With a slope of -0.335 on the fertility rate, a one-sd difference of 1.32 gives an expected change of $1.32 \times (-0.335) = -0.44$ points on the corruption scale. With a slope of 0.0121 on women's representation, a one-sd difference of 12.2 gives $12.2 \times 0.0121 = 0.15$<span style='display:inline-block;width:1.2em'></span> points. Development is the more powerful of the two continuous predictors, by roughly a factor of three --- and note that this comparison is only possible because we scaled each slope by its own variable's spread. Comparing -0.335 to 0.0121 directly would tell you nothing, since one is measured in children per woman and the other in percentage points.
...
question("Find the t-statistic and p-value for the Democracy vs. Autocracy coefficient in your model output. Which statements are true? Select all that apply.",
  ...
  answer("The p-value is below .05, so we reject the null hypothesis --- full democracies control corruption significantly better than autocracies, controlling for fertility and women's representation",
         correct = TRUE,
...
question("Which statement correctly describes what the model fit statistics tell us?",
  ...
  answer("The Multiple R-squared of 0.437 tells us the model explains about 43.7% of the variation in control of corruption across the 185 countries with complete data, and under roughly normal errors, about two-thirds of countries fall within about 0.76 points of their predicted value",
         correct = TRUE,
...
Two other things plausibly matter, and we will hold them constant rather than test them. An electoral gender quota is a deliberate intervention that changes what parties must do rather than what they are inclined to do. And representation accumulates over time, as parties build recruitment pipelines, women gain political experience, and voters grow accustomed to women as candidates --- so a parliament that has seated women for a century starts from a different place than one that seated its first in 2010.
...
- **first_woman_year**: the year the first woman entered that parliament, running from 1907 to 2021 and complete for all 193
...
Create **electoral_f** from **electoral_system**, with **plurality_majority** as the first level, labeled "Plurality/Majority", "Proportional", and "Mixed". Create **quota_f** from **gender_quota**, labeled "No quota" and "Quota". Create **years_c** by subtracting the mean of **years_since_first_woman** from itself. Three hints are available. Click "Submit Answer" to check your work.
...
# Hint 1 of 3
# A factor's first level is its reference category, and factor()
# takes the levels in the order you give them. Centering means
# subtracting a variable's mean from every one of its values.
```
...
# mean() needs na.rm = TRUE here, since some values are missing.
```
...
grade_this({
  ...
  if (!grepl("electoral_f\\s*(<-|=)", code_no_comments)) {
    fail("Assign the electoral-system factor to electoral_f.")
  }
  if (!grepl("quota_f\\s*(<-|=)", code_no_comments)) {
    fail("Assign the quota factor to quota_f.")
  }
  if (!grepl("years_c\\s*(<-|=)", code_no_comments)) {
    fail("Assign the centered history variable to years_c.")
  }
  ...
  if (!grepl("na\\.rm", code_no_comments)) {
    fail("mean() returns NA here unless you set na.rm = TRUE.")
  }

  if (is.null(.result) || !is.data.frame(.result)) {
    fail("Your last line should display the data frame, so the new variables can be checked.")
  }
  if (!all(c("electoral_f", "quota_f", "years_c") %in% names(.result))) {
    fail("All three new variables should be columns in ipu.")
  }
  if (!identical(levels(.result$electoral_f)[1], "Plurality/Majority")) {
    fail("Plurality/Majority should be the first level of electoral_f --- that is what makes it the reference category.")
  }
  if (!isTRUE(all.equal(mean(.result$years_c, na.rm = TRUE), 0, tolerance = 1e-6))) {
    fail("A centered variable has a mean of zero. Check that you subtracted the mean rather than something else.")
  }

  pass("Correct. Plurality/Majority is the reference category, and years_c is centered at 60.3 years.")
})
```
...
Write the code from scratch to estimate the model, assigning it to **model4**, then display the results. Two hints are available. Click "Submit Answer" to check your work.
...
grade_this({
  ...
  pass("Correct. Every coefficient on electoral_f is a comparison against plurality systems, and the intercept describes a country at the average length of women's parliamentary history.")
})
```
...
question("Which statements about the estimated coefficients are correct? Select all that apply.",
  ...
  answer("Countries using proportional representation are estimated to have about 7.77 percentage points more women in their legislature than countries using plurality systems, among countries with the same quota status and the same length of history",
         correct = TRUE,
         message = "This is the hypothesis test. The comparison is against plurality systems because that is the reference category you set."),
  answer("Countries using mixed systems are estimated to have about 5.54 percentage points more women than countries using plurality systems, among countries alike on the other predictors",
         correct = TRUE,
         message = "Mixed systems sit between proportional and plurality systems, closer to proportional."),
  answer("Among countries alike on the other predictors, for each year earlier that a parliament first seated a woman, it is predicted to seat about 0.09 percentage points more women",
         correct = TRUE,
  ...
  answer("Because proportional systems have the largest coefficient, electoral system explains more of the variation in women's representation than the other predictors",
         message = "Coefficient size and variation explained are different things. Comparing predictors on the same footing requires scaling each by its own spread, which the next section does."),
  allow_retry = TRUE,
  random_answer_order = TRUE,
...
- A one-standard-deviation difference in history: 24.3 years --- roughly the gap between a parliament that first seated a woman in 1966 and one that did so in 1942 --- gives $24.3 \times 0.094 = 2.28$ points, about **0.18**

The institutional predictors move representation roughly three times as much as a typical difference in history does. That is the case for the institutional account, and it is worth noticing that the raw coefficients would not have told you this: 7.77 and 0.094 are not comparable until each is scaled by the spread of the variable it belongs to.
...
question("What do the p-values tell us about the hypothesis and the controls? Select all that apply.",
  ...
  answer("The mixed-system coefficient has a p-value of .026, so mixed systems are also distinguishable from plurality systems",
         correct = TRUE,
         message = "This was not part of the hypothesis, but it is a real finding: mixed systems sit above plurality systems too."),
  answer("Both controls are statistically distinguishable from zero: quotas at p < .001 and history at p = .009",
         correct = TRUE,
         message = "Controls are included to hold constant, not to be tested --- but when they behave as theory predicts, that is reassuring about the model."),
...
question("What do the fit statistics tell us? Select all that apply.",
  ...
  answer("The Multiple R-squared of 0.3068 means the model explains about 30.7% of the variation in women's representation across the 173 countries with complete data",
         correct = TRUE,
         message = "Respectable for three predictors on a question this complex --- and it leaves roughly 69% unexplained."),
  answer("The residual standard error of 10.77 means a typical country's actual representation sits about 10.8 percentage points from what the model predicts",
         correct = TRUE,
...
grade_this({
  ...
  if (!all(diff(positions) > 0)) {
    fail("The labels are not in the correct order. They should appear as: Fertility Rate, Women in Legislature, Partial Democracy, Democracy --- matching the predictor order in the basic table output.")
  }
...
question("Examples 2 and 3 each set a reference category deliberately rather than accepting R's default. What does that choice change? Select all that apply.",
  correct = "Right. The reference category decides which comparisons the coefficients report --- and nothing else.",
  answer("It decides which comparison each factor coefficient reports: in Example 2, against autocracies; in Example 3, against plurality systems",
         correct = TRUE,
         message = "In both cases the default would have reported a comparison the hypothesis did not ask about --- Democracy in Example 2, mixed systems in Example 3, which R would take alphabetically."),
  answer("It changes what the intercept describes, since the intercept is the predicted outcome for the reference category when the other predictors are zero",
         correct = TRUE,
         message = "Example 2 showed this directly: the constant was 0.835 with Democracy as reference and 0.154 with Autocracy."),
  answer("It changes which differences appear statistically significant, because the tests compare each category against the reference",
         correct = TRUE,
  ...
  answer("It changes the coefficients on the continuous predictors in the model",
...
question("Across Examples 1, 2, and 3, which statement best captures what multiple regression adds compared to simple regression?",
  ...
  answer("In multiple regression, each coefficient describes the partial relationship between a predictor and the outcome among cases with the same values on the other included variables --- this can change both the size and significance of individual coefficients compared to simple regression",
         correct = TRUE,
...
1. [**All else equal.**]{.important-text} In multiple regression, every coefficient is a [**partial slope**]{.important-text} --- the expected change in $Y$ for a one-unit increase in that predictor, holding all other predictors constant. This qualifier changes interpretation fundamentally. A significant coefficient in a multiple regression means the variable is associated with the outcome *even after accounting for* the other variables in the model.
...
4. [**Assessing effect size.**]{.important-text} A statistically significant coefficient is not necessarily a practically important one. For continuous predictors, multiply the slope by one standard deviation of that variable --- $\text{sd}(X) * \hat{\beta}$ --- to find the expected change in the outcome across a typical difference in the predictor. For factor variables, compare the coefficient directly to the standard deviation of the outcome to gauge how large the group difference is relative to the outcome's overall variation.
...
<p class="tutorial-complete"
  ...
  Excellent work! You can now estimate and interpret multiple regression models with interval and nominal predictors --- and present your results in a professional table.
</p>
')
```
