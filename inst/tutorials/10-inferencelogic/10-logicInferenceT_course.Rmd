...
```{r ex-sample-dist, exercise=TRUE, exercise.cap="Run and observe 5", fig.alt="Bar plot of one random sample of 100 balls, with proportion on the y-axis and color on the x-axis. The bar for white balls is the taller of the two at just over 0.6; the bar for red balls reaches just under 0.4."}
...
p_sdist <- ggplot(sampling_means, aes(x = mean)) +
  geom_histogram(bins = 30, fill = "#CC79A7",
                 color = "black") +
...
ggplot(emp_rule, aes(x = x, y = y)) +
  ...
  geom_line(color = "#000000", linewidth = 1.1) +
  geom_vline(xintercept = -3:3, color = "#000000", linewidth = 0.7) +
  annotate(
    "segment", x = -1, xend = 1, y = 0.16, yend = 0.16,
    color = "#000000",
    arrow = arrow(ends = "both", length = unit(0.12, "in"))
  ) +
  annotate("label", x = 0, y = 0.173, label = "68% of data", size = 4.3,
           fill = "#FFFFFF", linewidth = 0) +
  annotate(
    "segment", x = -2, xend = 2, y = 0.105, yend = 0.105,
    color = "#000000",
    arrow = arrow(ends = "both", length = unit(0.12, "in"))
  ) +
  annotate("label", x = 0, y = 0.118, label = "95% of data", size = 4.3,
           fill = "#FFFFFF", linewidth = 0) +
  annotate(
  ...
  annotate("label", x = 0, y = 0.063, label = "99.7% of data", size = 4.3,
           fill = "#FFFFFF", linewidth = 0) +
  scale_x_continuous(
    breaks = -3:3,
    labels = -3:3
