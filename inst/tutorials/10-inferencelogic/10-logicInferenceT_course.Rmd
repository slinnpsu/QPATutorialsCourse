...
We use the [moderndive]{.package-name} package for this experiment. The package was designed specifically to teach statistical inference through virtual sampling, and it provides both the **bowl** data set and the `rep_sample_n()` function we will use to draw random samples from it.
...
a variable in one sample. It is what your data set looks like --- the
...
question(
  ...
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Hint: Ask what the x-axis shows. Are these individual responses from one data set, or statistics computed from many data sets?"
)
```
