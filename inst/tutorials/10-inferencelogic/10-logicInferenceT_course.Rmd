...
This experiment uses the [moderndive]{.package-name} package. The package was designed specifically to teach statistical inference through virtual sampling, and it provides both the **bowl** data set and the `rep_sample_n()` function we will use to draw random samples from it.
...
question(
  ...
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Hint: Ask two questions. Is any case missing? And does each bar count a case, or summarize a sample?"
)
```
...
question(
  ...
  allow_retry = TRUE,
  random_answer_order = TRUE,
  try_again = "Hint: The standard error measures spread across repeated samples. What would every one of those repeated samples have in common here?"
)
```
