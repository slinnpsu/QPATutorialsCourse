...
In this reading we introduce linear regression. Correlation tells us whether two variables move together; regression tells us how much --- how many more major bills a Congress passes for each additional seat the president's party holds, or how much a state's turnout differs as its population ages.
...
In this equation $Y$ is the outcome variable, $X$ is the predictor, $\alpha$ is the intercept (also called a constant), and $\beta$ is the slope. The intercept gives the expected value of $Y$ when $X = 0$. The slope is the average change in $Y$ when $X$ changes by 1 unit.

In the real world the data are messier. But this is to be expected. We know, for example, that other things also influence the number of important bills that are passed. There is also a *random component* influencing what we observe. Because many factors influence most things we care about, and because outcomes have a random element besides, we will never see all the points in a scatter plot fall perfectly on a line. But it still looks like there is a positive relationship. In fact, we can still draw a line through the cloud of points.
...
The answer is that we look at the residuals. We can draw a vertical line from each data point to the line. The length of this line --- the distance between each observed value of $Y$ and the value predicted by the line --- is that observation's **residual**. The plot below draws a set of lines through the scatter plot. You can see that the average length of the lines is larger in some of these examples than in others.
...
In fact, the line that best describes the relationship between seats and bills is the one that minimizes the sum of the squared residuals. That is, the vertical distance between the data points and the line is as small as possible. We minimize the squares because if we added up the positive and negative residuals they would cancel. (Taking the absolute value is also reasonable, but the math works best with squares.)

If you have a (really) good eye, you can tell that the line in the bottom right above is the best fitting line --- the one with the smallest sum of squared residuals.
...
The predicted value and the expected value are the same thing here: the line's estimate of the average outcome for cases with that value of the predictor.
...
Of course we know that not every point falls on the regression line. Our predicted value of **bills** will not (typically) match the actual number of **bills** passed. The residual we met earlier is the sample counterpart of the error in the population. The population error is denoted $\epsilon_i$; the sample residual is denoted $\hat{\epsilon}_i$ or sometimes $e_i$.
...
- The errors have a mean of zero at every value of $X$. This means the model does not systematically overpredict or underpredict $Y$ for particular values of the predictor --- not just on average across all the data, but at each place along the line.
...
- For the usual small-sample $t$-tests and confidence intervals to be exact, we assume the errors are normally distributed. Normality is not needed to estimate the line itself, and with larger samples the tests depend on it much less. When we say that errors are normally distributed, we mean that if you were to plot the distance from the actual $Y$-value to the expected $Y$-value based on the line, the resulting plot would be a normal distribution with a mean at 0 and a variance of $\sigma^2$:
...
One further condition is often listed alongside these, and it is worth keeping separate from them. If we want to read a regression as evidence about a causal relationship, we also need to have addressed the alternative explanations that could account for the association --- in particular, variables related to both the predictor and the outcome. This is not an assumption of the statistical model. A regression estimated with an omitted confounder is still a description of how $Y$ varies with $X$ in these data; what it cannot support is the claim that $X$ is what moves $Y$. Multiple regression can account for alternatives you have measured and included, and we take that up in a later lesson. It cannot account for the ones you have not.
...
Often the intercept will be of no interest to us because the predictor never takes the value of zero in the data. But we still include one. Leaving the intercept out would force the line through the origin, which is rarely justified, and it is the intercept that makes the residuals average to zero.
...
::: {.caution}
**Association, not necessarily causation.** A regression slope tells us how $Y$ changes on average as $X$ changes. It does not by itself show that changing $X$ would cause $Y$ to change. That claim needs the further conditions set out in the research process reading: a reason to expect $X$ to affect $Y$, evidence that $X$ comes first, and a design that addresses the alternative explanations that could account for the association.
:::
...
We calculate a residual as the difference between the observed and predicted value of $Y$ for a sample observation. Assume that **Presidential Popularity** has a value of 40% for one Congress and that the actual number of bills passed in that Congress was 12. Then the residual is given by:
...
How do we determine whether the population slope differs from zero? If $\beta = 0$, the predicted value of $Y$ does not increase or decrease as $X$ increases. We therefore test the null hypothesis that $\beta = 0$.
...
**Step 5. Draw a conclusion.** Recall that in order to determine whether to reject or fail to reject the null hypothesis in favor of the alternative hypothesis, we compare the p-value in step four to a pre-selected significance level: the threshold for how much evidence against the null hypothesis we require before rejecting it. Setting it at 0.05 means that, over many studies, we would reject a true null hypothesis five percent of the time. Assuming we set that level at 0.05, then if the p-value is less than 0.05 we reject $H_0$ --- the data are sufficiently surprising under the null that we conclude the true parameter value differs from zero. If the p-value is greater than 0.05, we fail to reject $H_0$. Similarly, if the confidence interval does not include zero, we can reject the null hypothesis. A $t$-statistic larger than about 2 in absolute value points to the same conclusion, but treat that as a rule of thumb: the exact cutoff depends on the number of observations, and where the rule of thumb and the p-value disagree, the p-value is the one to trust. If we can reject the null hypothesis, we conclude there is a statistically significant relationship between $X$ and $Y$.
...
The slope estimate for **filunif** is exactly the same as the mean difference above, 4.06. This is why a regression with one binary predictor gives us the same estimate of the difference between the two groups as a difference of means test.
...
Hypothesis testing can be conducted on both the intercept and slope in three different ways. If the p-value is less than 0.05, or the confidence interval around the estimate does not include zero, then we reject the null hypothesis that the population parameter equals zero. A $t$-statistic larger than about 2 in absolute value is a useful rough check, but the p-value is the exact test. Recall that a p-value tells us the probability of observing a result as extreme as ours --- or more extreme --- if the null hypothesis were true. It is not the probability that the null hypothesis is true.

We assess the fit of our model to know how well our model does at accounting for the variation in the outcome variable. $R^2$ tells us the proportion of the variation in $Y$ accounted for by the model. Often we multiply it by 100 and state "$X$% of the variation in the outcome variable is accounted for by the model." The residual standard error, also called the residual standard deviation, tells us the typical size of our prediction error, or the typical size of the residuals, measured in units of the outcome variable. We want to have a model with a larger $R^2$ and a smaller residual standard error. Keep in mind that a statistically significant slope and a large $R^2$ are answering different questions. A relationship can be statistically distinguishable from zero and precisely estimated --- and therefore highly significant --- while the model still leaves most of the variation in $Y$ unexplained. A significant slope provides evidence that the population slope differs from zero; $R^2$ tells you how much of the outcome's total variation the model accounts for.
