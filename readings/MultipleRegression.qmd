...
In this lesson you extend regression to models with more than one predictor. A single predictor can take credit that belongs elsewhere: countries where people stay in school longer have lower infant mortality, but those countries are also richer, and wealth buys clinics as well as classrooms. A model with schooling alone cannot tell the two apart. Multiple regression is how you begin to separate them.
...
The final type of relationship occurs when both $X_1$ and $X_2$ cause $Y$ (row 4) but $X_1$ and $X_2$ are not correlated. In this case, omitting $X_2$ does not distort the coefficient on $X_1$ merely because $X_2$ also helps explain $Y$. You can think of it this way: $X_1$ will not take credit for explaining variation in $Y$ due to $X_2$ if they are not correlated with each other.
...
<thead>
<tr><th scope="col">Term</th><th scope="col">Model 1</th></tr>
</thead>
<tbody>
<tr><th scope="row">Intercept</th><td>-13.08*</td></tr>
<tr class="se-row"><th scope="row"><span class="visually-hidden">Standard error for Intercept</span></th><td>(6.48)</td></tr>
<tr><th scope="row">POTUS Senate Seats</th><td>0.20*</td></tr>
<tr class="se-row"><th scope="row"><span class="visually-hidden">Standard error for POTUS Senate Seats</span></th><td>(0.07)</td></tr>
<tr><th scope="row">Congress Number</th><td>0.13*</td></tr>
<tr class="se-row"><th scope="row"><span class="visually-hidden">Standard error for Congress Number</span></th><td>(0.06)</td></tr>
...
Similarly, holding **pptysen** constant, each additional **Congress Number** predicts 0.13 more major bills, on average --- that is, holding seats constant, later Congresses are associated with slightly more legislation.

::: {.callout-note appearance="minimal" title="Pause and check"}
What does **holding Congress Number constant** mean in the interpretation of the **pptysen** coefficient? It does not mean Congress Number is unimportant. It means the coefficient describes how the model's predicted number of major bills changes as the president's party holds more Senate seats, with Congress Number held at the same value in the prediction. That is a comparison the model makes, not a set of real Congresses sitting side by side --- there is only one 110th Congress.
:::
...
<thead>
<tr><th scope="col">Coefficient</th><th scope="col">Estimate</th><th scope="col">Standard Error</th></tr>
</thead>
<tbody>
...
To summarize what has been covered so far, if your predictor(s) are measured at the interval level, you include them "as is" in the regression. If a predictor is measured at the nominal level, you include it as a series of binary variables for all but one category. That omitted category becomes the reference category. Interpretation of the slopes on each of these variables is made relative to the reference category. Which category you choose as the reference changes the comparison each coefficient represents, and can therefore change which individual coefficients are statistically significant. It does not change the fitted values or the overall fit of the model. Choose a reference category that makes the comparisons substantively meaningful --- a natural baseline, or a category your argument is about.
...
When deciding what to include in a multiple regression model, the choice of predictors should follow from the theoretical logic of the explanation. Consider what competing explanations might affect both the predictor of interest and the outcome. There is no single correct number of controls. Include variables because the theoretical logic of the model calls for them, not simply because they are available.
